...
# Try to import weight_generator, fallback to minicpm_o_2_6 if not in miniCPMo
try:
    from models.experimental.miniCPMo.tt.weight_generator import generate_dvae_weights
except ImportError:
...
def test_dvae_forward_pcc(device):
    ...
    try:
        ...
        pcc_threshold = 0.90
        pcc_passed, pcc_message = check_with_pcc(pt_output_nchw, tt_output_nchw, pcc=pcc_threshold)

        # Validate PCC
        if enable_gfsq:
            # With GFSQ quantization using PyTorch fallback, expect high PCC
            assert pcc_passed, f"GFSQ enabled: {pcc_message}"
            logger.info(f"✅ GFSQ quantization working correctly - {pcc_message}")
        else:
            # Without GFSQ, expect high PCC (reconstruction without quantization loss)
            assert pcc_passed, f"GFSQ disabled: {pcc_message}"
            logger.info(f"✅ DVAE forward pass {pcc_message}")
