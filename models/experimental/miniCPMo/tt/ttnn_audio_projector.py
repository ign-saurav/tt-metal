...
class TtnnAudioProjector:
    ...
    def load_weights(self, weights_dict: dict):
        ...
        """
        # Linear1 weights: [3584, 1024] → transpose to [1024, 3584] for TTNN
        if "linear1.weight" in weights_dict:
            linear1_weight = weights_dict["linear1.weight"]
            # Transpose for TTNN: [out_features, in_features] → [in_features, out_features]
            linear1_weight_ttnn = linear1_weight.t()
            self.linear1_weight = ttnn.from_torch(
                linear1_weight_ttnn,
            ...
            logger.warning("⚠️ linear1.weight not found")

        if "linear1.bias" in weights_dict:
            linear1_bias = weights_dict["linear1.bias"]
            self.linear1_bias = ttnn.from_torch(
                linear1_bias,
                dtype=ttnn.bfloat16,
                layout=ttnn.ROW_MAJOR_LAYOUT,
            ...
            logger.warning("⚠️ linear1.bias not found")

        # Linear2 weights: [3584, 3584] → transpose to [3584, 3584] for TTNN
        if "linear2.weight" in weights_dict:
            linear2_weight = weights_dict["linear2.weight"]
            # Transpose for TTNN: [out_features, in_features] → [in_features, out_features]
            linear2_weight_ttnn = linear2_weight.t()
            self.linear2_weight = ttnn.from_torch(
                linear2_weight_ttnn,
            ...
            logger.warning("⚠️ linear2.weight not found")

        if "linear2.bias" in weights_dict:
            linear2_bias = weights_dict["linear2.bias"]
            self.linear2_bias = ttnn.from_torch(
                linear2_bias,
                dtype=ttnn.bfloat16,
                layout=ttnn.ROW_MAJOR_LAYOUT,
            ...
            logger.warning("⚠️ linear2.bias not found")
    ...
    def forward(self, audio_features: ttnn.Tensor) -> ttnn.Tensor:
        ...
        x_reshaped = ttnn.reshape(
            x, [batch_size, hidden_dim, new_seq_len, self.pool_step], memory_config=ttnn.DRAM_MEMORY_CONFIG
        )

        # Average along the pooling dimension (last dim)
        x_pooled = ttnn.mean(
            x_reshaped, dim=-1, memory_config=ttnn.DRAM_MEMORY_CONFIG
        )  # Average over pool_step dimension
