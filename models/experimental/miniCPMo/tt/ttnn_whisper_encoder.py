...
def preprocess_encoder_inputs_minicpm(config, input_features, *, parameters, device):
    ...
    conv1_weight = ttnn.to_device(conv1_weight, device, memory_config=WHISPER_MEMORY_CONFIG)
    ...
    conv2_weight = ttnn.to_device(conv2_weight, device, memory_config=WHISPER_MEMORY_CONFIG)
    conv2_bias = ttnn.reshape(parameters["conv2"]["bias"], [1, 1, 1, config.d_model])
    conv2_bias = ttnn.to_device(conv2_bias, device, memory_config=WHISPER_MEMORY_CONFIG)
