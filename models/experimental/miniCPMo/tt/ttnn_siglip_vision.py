...
class TtSiglipVisionEmbeddings:
    ...
    def load_weights(self, weights_dict: Dict[str, torch.Tensor]):
        ...
        patch_bias_key = "embeddings.patch_embedding.bias"
        position_key = "embeddings.position_embedding.weight"
...
class TtSiglipEncoderLayer:
    ...
    def forward(self, hidden_states: ttnn.Tensor) -> ttnn.Tensor:
        ...
        # Normalize input shape to [1, 1, seq_len, hidden_size] as expected by TtLlamaImageAttention
        original_shape = hidden_states_pt.shape
        if len(original_shape) == 2:
            # Input is [seq_len, hidden_size] -> [1, 1, seq_len, hidden_size]
            hidden_states_pt = hidden_states_pt.unsqueeze(0).unsqueeze(0)
        elif len(original_shape) == 3:
            # Input is [batch, seq_len, hidden_size] -> [1, 1, seq_len, hidden_size]
            if original_shape[0] == 1:
                hidden_states_pt = hidden_states_pt.unsqueeze(0)  # [1, 1, seq_len, hidden_size]
            else:
                # Multiple batches - take first batch
                hidden_states_pt = hidden_states_pt[0:1].unsqueeze(0)  # [1, 1, seq_len, hidden_size]
        elif len(original_shape) == 4:
            # Already [1, 1, seq_len, hidden_size] or [batch, 1, seq_len, hidden_size]
            if original_shape[0] != 1 or original_shape[1] != 1:
                # Take first batch and first sequence if needed
                hidden_states_pt = hidden_states_pt[0:1, 0:1, :, :]

        seq_len = hidden_states_pt.shape[-2]
        hidden_size = hidden_states_pt.shape[-1]
        original_seq_len = seq_len

        # Pad sequence length to be divisible by 32 (required by TtLlamaImageAttention)
        num_pad_tokens = 0
        if seq_len % 32 != 0:
            num_pad_tokens = 32 - (seq_len % 32)
            # Pad along sequence dimension (dim=-2)
            hidden_states_pt = torch.nn.functional.pad(
                hidden_states_pt,
                (0, 0, 0, num_pad_tokens),  # (pad_left, pad_right, pad_top, pad_bottom) for last 2 dims
                mode="constant",
                value=0,
            )
            seq_len = hidden_states_pt.shape[-2]
        ...
        attn_state_dict = {
            "wq": {"weight": tt2torch_tensor(self.q_proj_weight), "bias": tt2torch_tensor(self.q_proj_bias)},
            "wk": {"weight": tt2torch_tensor(self.k_proj_weight), "bias": tt2torch_tensor(self.k_proj_bias)},
            "wv": {"weight": tt2torch_tensor(self.v_proj_weight), "bias": tt2torch_tensor(self.v_proj_bias)},
            "wo": {"weight": tt2torch_tensor(self.out_proj_weight), "bias": tt2torch_tensor(self.out_proj_bias)},
        }
        ...
        if num_pad_tokens > 0:
            hidden_states_pt = hidden_states_pt[:, :original_seq_len, :]  # Remove padded sequence tokens

        # Return to original shape before converting back to TTNN
        if len(original_shape) == 2:
            hidden_states_pt = hidden_states_pt.squeeze(0).squeeze(0)  # [seq_len, hidden_size]
        elif len(original_shape) == 3:
            hidden_states_pt = hidden_states_pt.squeeze(0)  # [1, seq_len, hidden_size] or [batch, seq_len, hidden_size]

        # Convert back to TTNN tensor
        # Ensure it has the right shape for ttnn.from_torch
        if len(hidden_states_pt.shape) == 2:
            hidden_states_pt = hidden_states_pt.unsqueeze(0)  # [1, seq_len, hidden_size]
        ...
        # MLP: FC1 -> GELU -> FC2
        self.fc1_weight = ttnn.transpose(self.fc1_weight, 1, 0)
        ...
        self.fc2_weight = ttnn.transpose(self.fc2_weight, 1, 0)
        hidden_states = ttnn.linear(
...
class TtSiglipVisionTransformer:
    ...
    def load_weights(self, weights_dict: Dict[str, torch.Tensor]):
        ...
        #     if k in ["embeddings.patch_embedding.weight", "embeddings.patch_embedding.bias", "embeddings.position_embedding.weight"]
        # }
    ...
    def forward(self, pixel_values: torch.Tensor, position_embeddings: ttnn.Tensor) -> ttnn.Tensor:
        """
