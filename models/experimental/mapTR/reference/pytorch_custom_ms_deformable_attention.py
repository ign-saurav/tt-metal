...
from models.experimental.mapTR.reference.utils import multi_scale_deformable_attn_pytorch

# Try to import MMCV components, fallback to PyTorch if not available
try:
    ...
    def constant_init(module, val, bias=0.0):
        """Constant initialization fallback."""
        if hasattr(module, "weight") and module.weight is not None:
    ...
    def deprecated_api_warning(*args, **kwargs):
        """Deprecated API warning decorator fallback."""
...
class CustomMSDeformableAttention(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Try CUDA acceleration if available, otherwise use PyTorch fallback
        if torch.cuda.is_available() and value.is_cuda:
            try:
                # Try to import MMCV CUDA extensions
                ...
                # Fallback to PyTorch implementation if CUDA extensions not available
                output = multi_scale_deformable_attn_pytorch(
                    value, spatial_shapes, sampling_locations, attention_weights
                )
        else:
            # Use PyTorch fallback for CPU or when CUDA not available
