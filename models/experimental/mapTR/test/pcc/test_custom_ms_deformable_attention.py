...
from models.experimental.mapTR.reference.pytorch_custom_ms_deformable_attention import (
    CustomMSDeformableAttention,
)
from models.experimental.mapTR.tt.tt_deformable_attention import TtCustomMSDeformableAttention
...
def create_maptr_model_parameters_attn(model: CustomMSDeformableAttention, input_tensor, device=None):
    ...
    parameters.conv_args = infer_ttnn_module_args(
        ...
        run_model=lambda model: model(
            ...
            identity=input_tensor[2],
            query_pos=input_tensor[3],
            reference_points=input_tensor[4],
            spatial_shapes=input_tensor[5],
            level_start_index=input_tensor[6],
        ),
        device=device,
    )
    return parameters
...
def test_maptr_custom_ms_deformable_attention(
    ...
    input_tensor = (query, value, identity, query_pos, reference_points, spatial_shapes, level_start_index)

    # Prepare TT model parameters
    parameter = create_maptr_model_parameters_attn(torch_model, input_tensor, device=device)
