...
class TtMapTRPerceptionTransformer(nn.Module):
    ...
    def _init_ttnn_layers(self):
        ...
        self.can_bus_mlp_weight1 = can_bus_params["0"]["weight"]
        self.can_bus_mlp_bias1 = can_bus_params["0"]["bias"]
        self.can_bus_mlp_weight2 = can_bus_params["1"]["weight"]
        self.can_bus_mlp_bias2 = can_bus_params["1"]["bias"]
    ...
    def _can_bus_mlp(self, can_bus: ttnn.Tensor) -> ttnn.Tensor:
        """TTNN CAN bus MLP implementation."""
        # First linear layer
        hidden = self._linear(can_bus, self.can_bus_mlp_weight1, self.can_bus_mlp_bias1)
        hidden = self._relu(hidden)

        # Second linear layer
        output = self._linear(hidden, self.can_bus_mlp_weight2, self.can_bus_mlp_bias2)
        output = self._relu(output)

        # Layer norm if enabled
        if self.can_bus_norm:
            output = self._layer_norm(output, self.can_bus_norm_weight, self.can_bus_norm_bias, self.can_bus_norm_eps)
    ...
    def attn_bev_encode(
        ...
        **kwargs,
        ...
        # Convert shift to TTNN tensor
        shift_torch = torch.from_numpy(np.stack([shift_x, shift_y], axis=1)).float()
        shift = ttnn.from_torch(shift_torch, dtype=ttnn.bfloat16, device=self.device)
        shift = ttnn.permute(shift, [1, 0])

        # Handle previous BEV features
        if prev_bev is not None:
            if ttnn.get_shape(prev_bev)[1] == bev_h * bev_w:
        ...
        can_bus = ttnn.from_torch(can_bus_torch, dtype=ttnn.bfloat16, device=self.device)
        can_bus_processed = self._can_bus_mlp(can_bus)
        ...
        for lvl, feat in enumerate(mlvl_feats):
            bs, num_cam, c, h, w = ttnn.get_shape(feat)
            spatial_shape = (h, w)
            ...
            if self.use_cams_embeds:
                ...
                cams_embed_expanded = ttnn.repeat(cams_embed_expanded, [1, h * w, bs, 1])
            ...
            level_embed_expanded = ttnn.repeat(level_embed_expanded, [num_cam, h * w, bs, 1])
        ...
        spatial_shapes = ttnn.from_torch(spatial_shapes_torch, dtype=ttnn.int32, device=self.device)

        # Calculate level start index
        spatial_shapes_np = np.array(spatial_shapes)
        level_start_index_np = np.concatenate(
            [np.zeros((1,), dtype=np.int64), np.cumprod(spatial_shapes_np[:, 0] * spatial_shapes_np[:, 1])[:-1]]
        )
        level_start_index = ttnn.from_torch(level_start_index_np, dtype=ttnn.int32, device=self.device)
    ...
    def lss_bev_encode(
        ...
        **kwargs,
        ...
        bs, c, _, _ = ttnn.get_shape(bev_embed)
