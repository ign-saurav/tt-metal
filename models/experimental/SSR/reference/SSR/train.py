...
def main(args):
    ...
    for epoch in range(args.epoch):
        ...
        for xtr, ytr, sr in train_loader:
            a3tr = nn.MaxPool2d(kernel_size=(args.imgsz // args.token_size), stride=(args.imgsz // args.token_size))(
                ytr
            )
            a2tr = nn.MaxPool2d(
                kernel_size=(args.imgsz // (args.token_size * 2)), stride=(args.imgsz // (args.token_size * 2))
            )(ytr)
            a1tr = nn.MaxPool2d(
                kernel_size=(args.imgsz // (args.token_size * 4)), stride=(args.imgsz // (args.token_size * 4))
            )(ytr)
        ...
        if args.eval:
            ...
            with torch.no_grad():
                for xte, yte, sr, name in test_loader:
                    a3te = nn.MaxPool2d(
                        kernel_size=(args.imgsz // args.token_size), stride=(args.imgsz // args.token_size)
                    )(yte)
            ...
            print(
                ...
                epoch + 1,
                "\ttrain loss:",
                "{:.4f}".format(train_loss / len(train_loader)),
                "\ttest loss",
                "{:.4f}".format(test_loss / len(test_loader)),
                "\tmaxf:",
                "{:.4f}".format(np.max(f_scores)),
                "\tIoU:",
                "{:.4f}".format(tp / (tp + fn + fp + 1e-20)),
                "\tTPR:",
                "{:.4f}".format(tp / (tp + fn + 1e-20)),
                "\tPSNR:",
                "{:.4f}".format(psnr / len(test_loader)),
                "\tSSIM:",
                "{:.4f}".format(ssim / len(test_loader)),
            )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--lr_slot", type=int, help="learning rate change point(related to batch size)", default=2000
    )
