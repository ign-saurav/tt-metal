...
def get_affine_transform(center, scale, rot, output_size, shift=np.array([0, 0], dtype=np.float32), inv=0):
    ...
    if inv:
        trans = cv2.getAffineTransform(
            np.ascontiguousarray(dst, dtype=np.float32), np.ascontiguousarray(src, dtype=np.float32)
        )
    else:
        trans = cv2.getAffineTransform(
            np.ascontiguousarray(src, dtype=np.float32), np.ascontiguousarray(dst, dtype=np.float32)
        )
