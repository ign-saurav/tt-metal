        ...
        for j in range(opt.num_classes):
            ...
            if include_wh:
                top_preds[j + 1] = np.concatenate(
                    [
                        top_preds[j + 1],
                        transform_preds(dets[i, inds, 15:17], c[i], s[i], (opt.output_w, opt.output_h)).astype(
                            np.float32
                        ),
                    ],
                    axis=1,
    ...
    for i in range(len(dets)):
        ...
        for cls_ind in dets[i].keys():
            ...
            for j in range(len(dets[i][cls_ind])):
                ...
                locations, rotation_y = ddd2locrot(center, alpha, dimensions, depth, calibs[0])
...
def multi_pose_post_process(dets, c, s, h, w):
    ...
    for i in range(dets.shape[0]):
        ...
        top_preds = (
            np.concatenate([bbox.reshape(-1, 4), dets[i, :, 4:5], pts.reshape(-1, 34)], axis=1)
            .astype(np.float32)
            .tolist()
        )
