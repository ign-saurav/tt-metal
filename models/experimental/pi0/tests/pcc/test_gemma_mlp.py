...
"""
PCC Test: Gemma MLP - TTNN vs PyTorch

Tests Gemma MLP blocks with both random and real checkpoint weights.

Usage:
    pytest test_pcc_gemma_mlp.py -v
    pytest test_pcc_gemma_mlp.py -v -k "pretrained_weight_true"   # Only real weights
    pytest test_pcc_gemma_mlp.py -v -k "pretrained_weight_false"  # Only random weights (fast)
    python test_pcc_gemma_mlp.py  # Standalone
"""
...
def create_random_mlp_weights(config: GemmaConfig) -> dict:
    """Create random MLP weights for fast testing."""
    return {
...
def main():
    """Standalone runner."""
    print("=" * 70)
    print("  Gemma MLP PCC Test (Checkpoint Weights)")
    print("=" * 70)
