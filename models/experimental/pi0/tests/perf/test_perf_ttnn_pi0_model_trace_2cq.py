...
"""
PI0 Performance Test - Trace + 2CQ

This test benchmarks the TTNN implementation with trace+2cq optimization.
...
Usage:
    pytest test_perf_ttnn_pi0_model_trace_2cq.py::test_pi0_inference_perf_trace_2cq -v -s
"""
...
def run_pi0_inference_trace_2cq(
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("  PI0 PERFORMANCE SUMMARY (Trace + 2CQ)")
    print("=" * 80)
...
@pytest.mark.parametrize(
    "device_params",
    [{"l1_small_size": 24576, "num_command_queues": 2, "trace_region_size": 8000000}],
    indirect=True,
)
def test_pi0_inference_perf_trace_2cq(device):
    """
    Pytest: PI0 inference performance test using trace+2cq.
    """
    checkpoint_path = Path(CHECKPOINT_PATH)
    if not checkpoint_path.exists():
        pytest.skip(f"Checkpoint not found: {checkpoint_path}")

    results = run_pi0_inference_trace_2cq(
        device,
...
def main():
    ...
    print("=" * 80)
    print("  PI0 TTNN PERFORMANCE TEST (Trace + 2CQ)")
    print("=" * 80)
