...
def create_stateless_maptr_pipeline_model(tt_model, fresh_parameters, tensor_tt, device, config, input_dict):
    def run(input_tensor: ttnn.Tensor) -> tuple:
        if isinstance(input_tensor, ttnn.Tensor):
            input_tensor = ttnn.deallocate(input_tensor)

        fresh_model = tt_model

        # if input_tensor.memory_config().buffer_type == ttnn.BufferType.L1:
        #     input_tensor = ttnn.to_memory_config(input_tensor, ttnn.DRAM_MEMORY_CONFIG)

        ttnn_img_feats = fresh_model.extract_feat(tensor_tt, input_dict["img_metas"])
...
def test_maptr_e2e_performant(
    ...
    pipeline_model = create_stateless_maptr_pipeline_model(
        tt_model, parameters, tensor_ttnn, device, config, input_dict
    )
    ...
    pipeline_input = ttnn.reshape(pipeline_input, (physical_height, width))
