...
try:
    ...
    raise ImportError(
        'Please run "pip install waymo-open-dataset-tf-2-1-0==1.2.0" ' "to install the official devkit first."
    )
...
class Waymo2KITTI(object):
    ...
    def save_calib(self, frame, file_idx, frame_idx):
        ...
        with open(
            f"{self.calib_save_dir}/{self.prefix}" + f"{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.txt", "w+"
        ) as fp_calib:
    ...
    def save_lidar(self, frame, file_idx, frame_idx):
        ...
        pc_path = (
            f"{self.point_cloud_save_dir}/{self.prefix}" + f"{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.bin"
        )
    ...
    def save_label(self, frame, file_idx, frame_idx):
        ...
        """
        fp_label_all = open(
            f"{self.label_all_save_dir}/{self.prefix}" + f"{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.txt", "w+"
        )
        id_to_bbox = dict()
        id_to_name = dict()
    ...
    def save_pose(self, frame, file_idx, frame_idx):
        ...
        np.savetxt(
            join(f"{self.pose_save_dir}/{self.prefix}" + f"{str(file_idx).zfill(3)}{str(frame_idx).zfill(3)}.txt"), pose
        )
