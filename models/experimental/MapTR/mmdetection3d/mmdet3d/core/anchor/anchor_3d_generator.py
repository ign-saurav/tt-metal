...
class Anchor3DRangeGenerator(object):
    ...
    def anchors_single_range(
        self, feature_size, anchor_range, scale=1, sizes=[[1.6, 3.9, 1.56]], rotations=[0, 1.5707963], device="cuda"
    ):
...
class AlignedAnchor3DRangeGenerator(Anchor3DRangeGenerator):
    ...
    def anchors_single_range(
        self, feature_size, anchor_range, scale, sizes=[[1.6, 3.9, 1.56]], rotations=[0, 1.5707963], device="cuda"
    ):
        """Generate anchors in a single range.
...
class AlignedAnchor3DRangeGeneratorPerCls(AlignedAnchor3DRangeGenerator):
    ...
    def __init__(self, **kwargs):
        ...
        assert len(self.scales) == 1, (
            "Multi-scale feature map levels are" + " not supported currently in this kind of anchor generator."
        )
    ...
    def multi_cls_grid_anchors(self, featmap_sizes, scale, device="cuda"):
        ...
        assert len(featmap_sizes) == len(self.sizes) == len(self.ranges), (
            "The number of different feature map sizes anchor sizes and " + "ranges should be the same."
        )

        multi_cls_anchors = []
        for i in range(len(featmap_sizes)):
            anchors = self.anchors_single_range(
                featmap_sizes[i], self.ranges[i], scale, self.sizes[i], self.rotations, device=device
            )
