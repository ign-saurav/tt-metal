...
def apply_3d_transformation(pcd, coord_type, img_meta, reverse=False):
    ...
    for op in flow:
        assert op in flow_mapping, f"This 3D data " f"transformation op ({op}) is not supported"
...
def extract_2d_info(img_meta, tensor):
    ...
    img_scale_factor = (
        tensor.new_tensor(img_meta["scale_factor"][:2]) if "scale_factor" in img_meta else tensor.new_tensor([1.0, 1.0])
    )
    ...
    img_crop_offset = (
        ...
        else tensor.new_tensor([0.0, 0.0])
    )
