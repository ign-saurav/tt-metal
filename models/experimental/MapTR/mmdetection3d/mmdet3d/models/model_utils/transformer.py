...
class GroupFree3DMHA(MultiheadAttention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(embed_dims, num_heads, attn_drop, proj_drop, dropout_layer, init_cfg, batch_first, **kwargs)

    def forward(
        self, query, key, value, identity, query_pos=None, key_pos=None, attn_mask=None, key_padding_mask=None, **kwargs
    ):
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        if hasattr(self, "operation_name"):
            if self.operation_name == "self_attn":
                ...
                raise NotImplementedError(f"{self.__class__.name} " f"can't be used as {self.operation_name}")
        else:
            value = value + query_pos
