...
class ImVoxelNet(BaseDetector):
    ...
    def extract_feat(self, img, img_metas):
        ...
        x = self.backbone(img)
        x = self.neck(x)[0]
        points = self.anchor_generator.grid_anchors([self.n_voxels[::-1]], device=img.device)[0][:, :3]
        volumes = []
        for feature, img_meta in zip(x, img_metas):
            img_scale_factor = (
                points.new_tensor(img_meta["scale_factor"][:2]) if "scale_factor" in img_meta.keys() else 1
            )
            img_flip = img_meta["flip"] if "flip" in img_meta.keys() else False
            img_crop_offset = (
                points.new_tensor(img_meta["img_crop_offset"]) if "img_crop_offset" in img_meta.keys() else 0
            )
            volume = point_sample(
                img_meta,
    ...
    def simple_test(self, img, img_metas):
        ...
        bbox_results = [
            bbox3d2result(det_bboxes, det_scores, det_labels) for det_bboxes, det_scores, det_labels in bbox_list
        ]
        return bbox_results
