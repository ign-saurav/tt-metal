...
class H3DNet(TwoStage3DDetector):
    ...
    def forward_train(
        ...
        if self.with_rpn:
            ...
            rpn_losses = self.rpn_head.loss(
                rpn_outs, *rpn_loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore, ret_target=True
            )
            ...
            proposal_list = self.rpn_head.get_bboxes(*proposal_inputs, use_nms=proposal_cfg.use_nms)
    ...
    def simple_test(self, points, img_metas, imgs=None, rescale=False):
        ...
        if self.with_rpn:
            ...
            feats_dict.update(rpn_outs)
            # Generate rpn proposals
            proposal_list = self.rpn_head.get_bboxes(points, rpn_outs, img_metas, use_nms=proposal_cfg.use_nms)
            feats_dict["proposal_list"] = proposal_list
        else:
            raise NotImplementedError

        return self.roi_head.simple_test(feats_dict, img_metas, points_cat, rescale=rescale)
    ...
    def aug_test(self, points, img_metas, imgs=None, rescale=False):
        ...
        aug_bboxes = []
        for feat_dict, pts_cat, img_meta in zip(feats_dict, points_cat, img_metas):
            if self.with_rpn:
                proposal_cfg = self.test_cfg.rpn
            ...
            bbox_results = self.roi_head.simple_test(
                feat_dict, self.test_cfg.rcnn.sample_mod, img_meta, pts_cat, rescale=rescale
            )
