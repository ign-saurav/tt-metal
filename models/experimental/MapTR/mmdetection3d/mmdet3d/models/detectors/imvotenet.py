        ...
        **kwargs,
        ...
        if points is None:
            ...
            losses_joint = self.pts_bbox_head_joint.loss(
                bbox_preds_joints, *loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore
            )
    ...
    def forward_test(self, points=None, img_metas=None, img=None, bboxes_2d=None, **kwargs):
        ...
        if points is None:
            ...
            if num_augs != len(img_metas):
                raise ValueError(f"num of augmentations ({len(img)}) " f"!= num of image meta ({len(img_metas)})")
            ...
            if num_augs == 1:
                ...
                assert img[0].size(0) == 1, (
                    "aug test does not support " "inference with batch size " f"{img[0].size(0)}"
                )
            ...
            if num_augs != len(img_metas):
                raise ValueError(
                    "num of augmentations ({}) != num of image meta ({})".format(len(points), len(img_metas))
                )

            if num_augs == 1:
                return self.simple_test(
                    points[0], img_metas[0], img[0], bboxes_2d=bboxes_2d[0] if bboxes_2d is not None else None, **kwargs
                )
            else:
