...
class EncoderDecoder3D(Base3DSegmentor):
    ...
    def _sliding_patch_generation(
        self, points, num_points, block_size, sample_rate=0.5, use_normalized_coord=False, eps=1e-3
    ):
        ...
        for idx_y in range(num_grid_y):
            ...
            for idx_x in range(num_grid_x):
                ...
                point_batches = self._input_generation(
                    coords[choices], cur_center, coord_max, feats[choices], use_normalized_coord=use_normalized_coord
                )
    ...
    def inference(self, points, img_metas, rescale):
        ...
        if self.test_cfg.mode == "slide":
            seg_logit = torch.stack(
                [self.slide_inference(point, img_meta, rescale) for point, img_meta in zip(points, img_metas)], 0
            )
        else:
