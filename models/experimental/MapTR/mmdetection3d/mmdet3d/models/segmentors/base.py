...
class Base3DSegmentor(BaseSegmentor):
    ...
    def with_regularization_loss(self):
        ...
        return hasattr(self, "loss_regularization") and self.loss_regularization is not None
    ...
    def forward_test(self, points, img_metas, **kwargs):
        ...
        if num_augs != len(img_metas):
            raise ValueError(f"num of augmentations ({len(points)}) != " f"num of image meta ({len(img_metas)})")
    ...
    def forward(self, return_loss=True, **kwargs):
        ...
        if return_loss:
            raise NotImplementedError(
                "3D segmentor training is disabled in this embedded MapTR reference. Use return_loss=False for inference."
            )
        return self.forward_test(**kwargs)

    def show_results(self, data, result, palette=None, out_dir=None, ignore_index=None):
        ...
        for batch_id in range(len(result)):
            ...
            show_seg_result(points, None, pred_sem_mask, out_dir, file_name, palette, ignore_index, show=True)
