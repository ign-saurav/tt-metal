...
class H3DBboxHead(BaseModule):
    ...
    def forward(self, feats_dict, sample_mod):
        ...
        surface_center_feature_pred = torch.cat(
            (
                surface_center_feature_pred.new_zeros((batch_size, 6, surface_center_feature_pred.shape[2])),
        ...
        surface_xyz, surface_features, _ = self.surface_center_matcher(
            surface_center_pred, surface_center_feature_pred, target_xyz=obj_surface_center
        )
        ...
        refine_decode_res = self.bbox_coder.split_pred(
            bbox_predictions[:, : self.num_classes + 2], bbox_predictions[:, self.num_classes + 2 :], aggregated_points
        )
    ...
    def loss(
        ...
        primitive_sem_loss = self.cues_semantic_loss(
            objectness_scores_sem.transpose(2, 1), cues_sem_label, weight=cues_mask, avg_factor=cues_mask.sum() + 1e-6
        )
        ...
        primitive_matching_loss = (
            (objectness_loss_refine * cues_match_mask).sum() / (cues_match_mask.sum() + 1e-6) * 0.5
        )
        primitive_sem_matching_loss = (
            (objectness_loss_refine * proposal_objectness_mask).sum() / (proposal_objectness_mask.sum() + 1e-6) * 0.5
        )
        ...
        pred_obj_surface_center = (
            pred_obj_surface_center.reshape(batch_size, -1, 6, 3).transpose(1, 2).reshape(batch_size, -1, 3)
        )
        pred_obj_line_center = (
            pred_obj_line_center.reshape(batch_size, -1, 12, 3).transpose(1, 2).reshape(batch_size, -1, 3)
        )
    ...
    def get_bboxes(self, points, bbox_preds, input_metas, rescale=False, suffix=""):
        ...
        for b in range(batch_size):
            bbox_selected, score_selected, labels = self.multiclass_nms_single(
                obj_scores[b], sem_scores[b], bbox3d[b], points[b, ..., :3], input_metas[b]
            )
            bbox = input_metas[b]["box_type_3d"](
                bbox_selected, box_dim=bbox_selected.shape[-1], with_yaw=self.bbox_coder.with_rot
            )
    ...
    def multiclass_nms_single(self, obj_scores, sem_scores, bbox, points, input_meta):
        ...
        bbox = input_meta["box_type_3d"](
            bbox, box_dim=bbox.shape[-1], with_yaw=self.bbox_coder.with_rot, origin=(0.5, 0.5, 0.5)
        )
    ...
    def get_proposal_stage_loss(
        ...
        objectness_loss = self.objectness_loss(
            bbox_preds["obj_scores" + suffix].transpose(2, 1), objectness_targets, weight=objectness_weights
        )

        # calculate center loss
        source2target_loss, target2source_loss = self.center_loss(
            bbox_preds["center" + suffix], center_targets, src_weight=box_loss_weights, dst_weight=valid_gt_weights
        )
        center_loss = source2target_loss + target2source_loss

        # calculate direction class loss
        dir_class_loss = self.dir_class_loss(
            bbox_preds["dir_class" + suffix].transpose(2, 1), dir_class_targets, weight=box_loss_weights
        )
        ...
        size_class_loss = self.size_class_loss(
            bbox_preds["size_class" + suffix].transpose(2, 1), size_class_targets, weight=box_loss_weights
        )
        ...
        semantic_loss = self.semantic_loss(
            bbox_preds["sem_scores" + suffix].transpose(2, 1), mask_targets, weight=box_loss_weights
        )
    ...
    def get_targets(
        self, points, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask=None, pts_instance_mask=None, bbox_preds=None
    ):
    ...
    def get_targets_single(
        ...
        dist1, dist2, ind1, _ = chamfer_distance(
            aggregated_points.unsqueeze(0), gt_center.unsqueeze(0), reduction="none"
        )
        # Set assignment
        ...
        dist_surface, _, surface_ind, _ = chamfer_distance(
            obj_surface_center, pred_surface_center.unsqueeze(0), reduction="none"
        )
        ...
        euclidean_dist_obj_surface = torch.sqrt(((pred_obj_surface_center - surface_sel) ** 2).sum(dim=-1) + 1e-6)
        ...
        objectness_label_surface[
            (euclidean_dist_obj_surface < self.train_cfg["label_surface_threshold"])
            * (euclidean_dist_surface < self.train_cfg["mask_surface_threshold"])
        ] = 1
        objectness_label_surface_sem[
            (euclidean_dist_obj_surface < self.train_cfg["label_surface_threshold"])
            * (euclidean_dist_surface < self.train_cfg["mask_surface_threshold"])
            * (surface_sel_sem == surface_sel_sem_gt)
        ] = 1
