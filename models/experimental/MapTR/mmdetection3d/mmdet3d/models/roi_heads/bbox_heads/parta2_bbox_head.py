...
class PartA2BboxHead(BaseModule):
    ...
    def __init__(
        ...
        loss_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, reduction="none", loss_weight=1.0),
        init_cfg=None,
    ):
        ...
        for i, channel in enumerate(merge_conv_channels):
            merge_conv.append(
                make_sparse_convmodule(
                    merge_conv_channel_last, channel, 3, padding=1, norm_cfg=norm_cfg, indice_key="rcnn_down0"
                )
            )
        ...
        for i, channel in enumerate(down_conv_channels):
            conv_down.append(
                make_sparse_convmodule(
                    down_conv_channel_last, channel, 3, padding=1, norm_cfg=norm_cfg, indice_key="rcnn_down1"
                )
            )
            down_conv_channel_last = channel

        self.conv_down.add_module("merge_conv", spconv.SparseSequential(*merge_conv))
        self.conv_down.add_module("max_pool3d", spconv.SparseMaxPool3d(kernel_size=2, stride=2))
        self.conv_down.add_module("down_conv", spconv.SparseSequential(*conv_down))
        ...
        for k in range(1, len(shared_fc_channels)):
            shared_fc_list.append(
                ConvModule(
                    pre_channel, shared_fc_channels[k], 1, padding=0, conv_cfg=conv_cfg, norm_cfg=norm_cfg, inplace=True
                )
            )
        ...
        for k in range(0, len(cls_channels)):
            cls_layers.append(
                ConvModule(
                    pre_channel, cls_channels[k], 1, padding=0, conv_cfg=conv_cfg, norm_cfg=norm_cfg, inplace=True
                )
            )
        ...
        for k in range(0, len(reg_channels)):
            reg_layers.append(
                ConvModule(
                    pre_channel, reg_channels[k], 1, padding=0, conv_cfg=conv_cfg, norm_cfg=norm_cfg, inplace=True
                )
            )
            pre_channel = reg_channels[k]
        reg_layers.append(
            ConvModule(pre_channel, self.bbox_coder.code_size, 1, padding=0, conv_cfg=conv_cfg, act_cfg=None)
        )
        if dropout_ratio >= 0:
    ...
    def forward(self, seg_feats, part_feats):
        ...
        part_features = spconv.SparseConvTensor(part_features, coords, sparse_shape, rcnn_batch_size)
        seg_features = spconv.SparseConvTensor(seg_features, coords, sparse_shape, rcnn_batch_size)
        ...
        shared_feature = spconv.SparseConvTensor(merged_feature, coords, sparse_shape, rcnn_batch_size)
        ...
        cls_score = self.conv_cls(shared_feature).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, 1)
        bbox_pred = self.conv_reg(shared_feature).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, C)

        return cls_score, bbox_pred

    def loss(
        self, cls_score, bbox_pred, rois, labels, bbox_targets, pos_gt_bboxes, reg_mask, label_weights, bbox_weights
    ):
        """Coumputing losses.
        ...
        if pos_inds.any() == 0:
            ...
            loss_bbox = self.loss_bbox(
                pos_bbox_pred.unsqueeze(dim=0), bbox_targets.unsqueeze(dim=0), bbox_weights_flat.unsqueeze(dim=0)
            )
            ...
            if self.with_corner_loss:
                ...
                batch_anchors[..., 0:3] = 0
                # decode boxes
                pred_boxes3d = self.bbox_coder.decode(batch_anchors, pos_bbox_pred.view(-1, code_size)).view(
                    -1, code_size
                )

                pred_boxes3d[..., 0:3] = rotation_3d_in_axis(
                    pred_boxes3d[..., 0:3].unsqueeze(1), (pos_rois_rotation + np.pi / 2), axis=2
                ).squeeze(1)
    ...
    def get_targets(self, sampling_results, rcnn_train_cfg, concat=True):
        ...
        targets = multi_apply(
            self._get_target_single, pos_bboxes_list, pos_gt_bboxes_list, iou_list, cfg=rcnn_train_cfg
        )
    ...
    def get_bboxes(self, rois, cls_score, bbox_pred, class_labels, class_pred, img_metas, cfg=None):
        ...
        rcnn_boxes3d[..., 0:3] = rotation_3d_in_axis(
            rcnn_boxes3d[..., 0:3].unsqueeze(1), (roi_ry + np.pi / 2), axis=2
        ).squeeze(1)
        ...
        for batch_id in range(batch_size):
            ...
            selected = self.multi_class_nms(
                cur_box_prob, cur_rcnn_boxes3d, cfg.score_thr, cfg.nms_thr, img_metas[batch_id], cfg.use_rotate_nms
            )
            ...
            result_list.append(
                (
                    img_metas[batch_id]["box_type_3d"](selected_bboxes, self.bbox_coder.code_size),
                    selected_scores,
