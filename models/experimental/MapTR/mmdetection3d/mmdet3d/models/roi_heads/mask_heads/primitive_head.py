...
class PrimitiveHead(BaseModule):
    ...
    def forward(self, feats_dict, sample_mod):
        ...
        if sample_mod == "vote":
            ...
            sample_indices = torch.randint(
                0, num_seed, (batch_size, self.num_proposal), dtype=torch.int32, device=seed_points.device
            )
        else:
    ...
    def get_targets(
        ...
        (point_mask, point_sem, point_offset) = multi_apply(
            self.get_targets_single, points, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask, pts_instance_mask
        )
        ...
        gt_primitive_semantic = (
            seed_gt_sem[:, :, 3 : 3 + self.num_dims].view(batch_size * num_proposal, 1, self.num_dims).contiguous()
        )
    ...
    def get_targets_single(self, points, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask=None, pts_instance_mask=None):
        ...
        for i, i_instance in enumerate(instance_labels):
            ...
            if (
                ...
                and plane_lower_temp[0] + plane_lower_temp[1] < self.train_cfg["lower_thresh"]
            ):
            ...
            if self.primitive_mode == "line":
                ...
                point_mask, point_offset, point_sem = self._assign_primitive_line_targets(
                    point_mask,
                    point_offset,
                    point_sem,
                    coords[selected],
            ...
            if (
                ...
                point_mask, point_offset, point_sem = self._assign_primitive_surface_targets(
                    point_mask,
                    point_offset,
                    point_sem,
                    coords[selected],
            ...
            plane_left_temp = self._get_plane_fomulation(
                cur_corners[2] - cur_corners[3], cur_corners[3] - cur_corners[0], cur_corners[0]
            )
            ...
            if plane_left_temp[2] < self.train_cfg["lower_thresh"]:
                ...
                plane_right = points.new_tensor(
                    [plane_left_temp[0], plane_left_temp[1], plane_left_temp[2], -refined_distance.mean()]
                )
            else:
            ...
            plane_front_temp = self._get_plane_fomulation(
                cur_corners[0] - cur_corners[4], cur_corners[4] - cur_corners[5], cur_corners[5]
            )
            ...
            if plane_front_temp[2] < self.train_cfg["lower_thresh"]:
                ...
                plane_back = points.new_tensor(
                    [plane_front_temp[0], plane_front_temp[1], plane_front_temp[2], -torch.mean(refined_distance)]
                )
            else:
                raise NotImplementedError("Normal vector of the plane should be horizontal!")
    ...
    def check_horizon(self, points):
        ...
        """
        return (
            (points[0][-1] == points[1][-1]) and (points[1][-1] == points[2][-1]) and (points[2][-1] == points[3][-1])
        )
    ...
    def match_point2line(self, points, corners, with_yaw, mode="bottom"):
        ...
        if with_yaw:
            ...
            for pair_index in corners_pair[mode]:
                selected = (
                    self.point2line_dist(points, corners[pair_index[0]], corners[pair_index[1]])
    ...
    def compute_primitive_loss(
        ...
        center_loss = self.center_loss(
            vote_xyz_reshape, gt_primitive_center, dst_weight=gt_primitive_mask.view(batch_size * num_proposal, 1)
        )[1]
        ...
        if self.primitive_mode != "line":
            ...
            size_loss = self.semantic_reg_loss(
                size_xyz_reshape, gt_primitive_semantic, dst_weight=gt_primitive_mask.view(batch_size * num_proposal, 1)
            )[1]
        else:
    ...
    def _assign_primitive_surface_targets(
        self, point_mask, point_offset, point_sem, coords, indices, cls_label, corners, with_yaw, mode="bottom"
    ):
        ...
        if self.primitive_mode == "z":
            ...
            if with_yaw:
                ...
                point_sem[indices] = point_sem.new_tensor(
                    [center[0], center[1], center[2], corners[pair_index[1], 2] - corners[pair_index[0], 2], cls_label]
                )
            else:
                center = point_mask.new_tensor([coords[:, 0].mean(), coords[:, 1].mean(), corners[:, 2].mean()])
                point_sem[indices] = point_sem.new_tensor(
                    [center[0], center[1], center[2], corners[:, 2].max() - corners[:, 2].min(), cls_label]
                )
