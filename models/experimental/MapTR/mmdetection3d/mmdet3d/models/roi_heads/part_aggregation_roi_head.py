...
class PartAggregationROIHead(Base3DRoIHead):
    ...
    def __init__(
        ...
        super(PartAggregationROIHead, self).__init__(
            bbox_head=bbox_head, train_cfg=train_cfg, test_cfg=test_cfg, init_cfg=init_cfg
        )
    ...
    def forward_train(self, feats_dict, voxels_dict, img_metas, proposal_list, gt_bboxes_3d, gt_labels_3d):
        ...
        if self.with_semantic:
            semantic_results = self._semantic_forward_train(
                feats_dict["seg_features"], voxels_dict, gt_bboxes_3d, gt_labels_3d
            )
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(
                feats_dict["seg_features"], semantic_results["part_feats"], voxels_dict, sample_results
            )
    ...
    def _bbox_forward(self, seg_feats, part_feats, voxels_dict, rois):
        ...
        pooled_seg_feats = self.seg_roi_extractor(
            seg_feats, voxels_dict["voxel_centers"], voxels_dict["coors"][..., 0], rois
        )
        pooled_part_feats = self.part_roi_extractor(
            part_feats, voxels_dict["voxel_centers"], voxels_dict["coors"][..., 0], rois
        )
