...
class VoteHead(BaseModule):
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        if sample_mod == "vote":
            ...
            sample_indices = seed_points.new_tensor(
                torch.randint(0, num_seed, (batch_size, self.num_proposal)), dtype=torch.int32
            )
            aggregation_inputs = dict(points_xyz=vote_points, features=vote_features, indices=sample_indices)
    ...
    def loss(
        ...
        objectness_loss = self.objectness_loss(
            bbox_preds["obj_scores"].transpose(2, 1), objectness_targets, weight=objectness_weights
        )

        # calculate center loss
        source2target_loss, target2source_loss = self.center_loss(
            bbox_preds["center"], center_targets, src_weight=box_loss_weights, dst_weight=valid_gt_weights
        )
        center_loss = source2target_loss + target2source_loss

        # calculate direction class loss
        dir_class_loss = self.dir_class_loss(
            bbox_preds["dir_class"].transpose(2, 1), dir_class_targets, weight=box_loss_weights
        )
        ...
        size_class_loss = self.size_class_loss(
            bbox_preds["size_class"].transpose(2, 1), size_class_targets, weight=box_loss_weights
        )
        ...
        semantic_loss = self.semantic_loss(
            bbox_preds["sem_scores"].transpose(2, 1), mask_targets, weight=box_loss_weights
        )
        ...
        if self.iou_loss:
            ...
            corners_target = self.bbox_coder.decode_corners(
                assigned_center_targets, size_res_targets, one_hot_size_targets_expand
            )
    ...
    def get_targets_single(
        self, points, gt_bboxes_3d, gt_labels_3d, pts_semantic_mask=None, pts_instance_mask=None, aggregated_points=None
    ):
        ...
        distance1, _, assignment, _ = chamfer_distance(
            aggregated_points.unsqueeze(0), center_targets.unsqueeze(0), reduction="none"
        )
