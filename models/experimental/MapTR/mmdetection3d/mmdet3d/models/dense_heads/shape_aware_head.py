...
class BaseShapeHead(BaseModule):
            ...
            if use_direction_classifier:
                self.init_cfg = dict(
                    ...
                    override=[
                        ...
                        dict(type="Normal", name="conv_dir_cls", std=0.01, bias_prob=0.01),
                    ],
                )
            else:
                self.init_cfg = dict(
    ...
    def forward(self, x):
        ...
        if self.use_direction_classifier:
            ...
            dir_cls_preds = (
                dir_cls_preds.view(-1, self.num_base_anchors, 2, H, W).permute(0, 1, 3, 4, 2).reshape(B, -1, 2)
            )
...
class ShapeAwareHead(Anchor3DHead):
    ...
    def forward_single(self, x):
        ...
        assert len(self.featmap_sizes) == len(self.anchor_generator.ranges), (
            "Length of feature map sizes must be equal to length of " + "different ranges of anchor generator."
        )
    ...
    def get_bboxes(self, cls_scores, bbox_preds, dir_cls_preds, input_metas, cfg=None, rescale=False):
        ...
        for img_id in range(len(input_metas)):
            ...
            proposals = self.get_bboxes_single(
                cls_score_list, bbox_pred_list, dir_cls_pred_list, mlvl_anchors, input_meta, cfg, rescale
            )
            result_list.append(proposals)
        return result_list

    def get_bboxes_single(
        self, cls_scores, bbox_preds, dir_cls_preds, mlvl_anchors, input_meta, cfg=None, rescale=False
    ):
