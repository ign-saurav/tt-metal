...
class AnchorFreeMono3DHead(BaseMono3DDenseHead):
    ...
    def __init__(
        ...
        cls_branch=(128, 64),
        reg_branch=((128, 64), (128, 64), (64,), (64,), ()),  # offset  # depth  # size  # rot  # velo
        dir_branch=(64,),
        attr_branch=(64,),
        conv_cfg=None,
        norm_cfg=None,
        ...
        assert len(reg_branch) == len(group_reg_dims), (
            "The number of " "element in reg_branch and group_reg_dims should be the same."
        )
    ...
    def _init_predictor(self):
        ...
        for i in range(len(self.group_reg_dims)):
            ...
            if len(reg_branch_channels) > 0:
                self.conv_reg_prevs.append(
                    self._init_branch(conv_channels=reg_branch_channels, conv_strides=(1,) * len(reg_branch_channels))
                )
        ...
        if self.pred_attrs:
            self.conv_attr_prev = self._init_branch(
                conv_channels=self.attr_branch, conv_strides=(1,) * len(self.attr_branch)
            )
