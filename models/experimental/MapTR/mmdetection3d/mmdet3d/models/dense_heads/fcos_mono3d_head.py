...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    @staticmethod
    def get_direction_target(reg_targets, dir_offset=0, num_bins=2, one_hot=True):
        ...
        if one_hot:
            dir_targets = torch.zeros(
                *list(dir_cls_targets.shape), num_bins, dtype=reg_targets.dtype, device=dir_cls_targets.device
            )
    ...
    def loss(
        ...
        labels_3d, bbox_targets_3d, centerness_targets, attr_targets = self.get_targets(
            all_level_points, gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths, attr_labels
        )
        ...
        flatten_bbox_preds = [
            bbox_pred.permute(0, 2, 3, 1).reshape(-1, sum(self.group_reg_dims)) for bbox_pred in bbox_preds
        ]
        ...
        if num_pos > 0:
            ...
            if self.use_direction_classifier:
                loss_dir = self.loss_dir(
                    pos_dir_cls_preds, pos_dir_cls_targets, equal_weights, avg_factor=equal_weights.sum()
                )
            ...
            if self.pred_attrs:
                loss_attr = self.loss_attr(
                    pos_attr_preds, pos_attr_targets, pos_centerness_targets, avg_factor=pos_centerness_targets.sum()
                )
    ...
    @force_fp32(apply_to=("cls_scores", "bbox_preds", "dir_cls_preds", "attr_preds", "centernesses"))
    def get_bboxes(
        self, cls_scores, bbox_preds, dir_cls_preds, attr_preds, centernesses, img_metas, cfg=None, rescale=None
    ):
        ...
        for img_id in range(len(img_metas)):
            ...
            if self.use_direction_classifier:
                ...
                dir_cls_pred_list = [
                    cls_scores[i][img_id].new_full([2, *cls_scores[i][img_id].shape[1:]], 0).detach()
            ...
            if self.pred_attrs:
                ...
                attr_pred_list = [
                    cls_scores[i][img_id]
                    .new_full([self.num_attrs, *cls_scores[i][img_id].shape[1:]], self.attr_background_label)
                    .detach()
                    for i in range(num_levels)
                ]
    ...
    def _get_bboxes_single(
        ...
        if mlvl_bboxes.shape[0] > 0:
            dir_rot = limit_period(mlvl_bboxes[..., 6] - self.dir_offset, 0, np.pi)
            mlvl_bboxes[..., 6] = dir_rot + self.dir_offset + np.pi * mlvl_dir_scores.to(mlvl_bboxes.dtype)

        cam_intrinsic = mlvl_centers2d.new_zeros((4, 4))
        cam_intrinsic[: view.shape[0], : view.shape[1]] = mlvl_centers2d.new_tensor(view)
        mlvl_bboxes[:, 6] = (
            torch.atan2(mlvl_centers2d[:, 0] - cam_intrinsic[0, 2], cam_intrinsic[0, 0]) + mlvl_bboxes[:, 6]
        )
    ...
    def get_targets(
        ...
        if attr_labels_list is None:
            attr_labels_list = [
                gt_labels.new_full(gt_labels.shape, self.attr_background_label) for gt_labels in gt_labels_list
            ]

        # get labels and bbox_targets of each image
        _, _, labels_3d_list, bbox_targets_3d_list, centerness_targets_list, attr_targets_list = multi_apply(
