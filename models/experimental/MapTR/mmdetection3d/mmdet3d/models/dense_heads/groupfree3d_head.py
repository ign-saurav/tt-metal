...
class GroupFree3DHead(BaseModule):
    ...
    def __init__(
        ...
        num_decoder_layers,
        transformerlayers,
        decoder_self_posembeds=dict(type="ConvBNPositionalEncoding", input_channel=6, num_pos_feats=288),
        decoder_cross_posembeds=dict(type="ConvBNPositionalEncoding", input_channel=3, num_pos_feats=288),
        train_cfg=None,
        test_cfg=None,
        num_proposal=128,
        pred_layer_cfg=None,
    ...
    def loss(
        ...
        sampling_objectness_loss = self.sampling_objectness_loss(
            sampling_obj_score, 1 - sampling_targets.reshape(-1), sampling_weights.reshape(-1), avg_factor=batch_size
        )
        ...
        for prefix in prefixes:
            ...
            center_loss = self.center_loss(
                bbox_preds[f"{prefix}center"], assigned_center_targets, weight=box_loss_weights_expand
            )
            losses[f"{prefix}center_loss"] = center_loss / num_stages

            # calculate direction class loss
            dir_class_loss = self.dir_class_loss(
                bbox_preds[f"{prefix}dir_class"].transpose(2, 1), dir_class_targets, weight=box_loss_weights
            )
            ...
            if self.size_cls_agnostic:
                ...
                size_reg_loss = self.size_reg_loss(
                    bbox_preds[f"{prefix}size"], assigned_size_targets, weight=box_loss_weights_expand
                )
                ...
                size_class_loss = self.size_class_loss(
                    bbox_preds[f"{prefix}size_class"].transpose(2, 1), size_class_targets, weight=box_loss_weights
                )
            ...
            semantic_loss = self.semantic_loss(
                bbox_preds[f"{prefix}sem_scores"].transpose(2, 1), mask_targets, weight=box_loss_weights
            )
    ...
    def get_targets_single(
        ...
        if self.bbox_coder.with_rot:
            ...
            for i in range(gt_labels_3d.shape[0]):
                ...
                for j in range(self.gt_per_seed):
                    ...
                    if j == 0:
                        vote_targets_tmp[column_indices, : 3 * self.gt_per_seed] = votes[column_indices].repeat(
                            1, self.gt_per_seed
                        )
        ...
        object_assignment_one_hot.scatter_(1, object_assignment.unsqueeze(-1), 1)  # (num_seed, gt_num)

        delta_xyz = seed_points.unsqueeze(1) - gt_bboxes_3d.gravity_center.unsqueeze(0)  # (num_seed, gt_num, 3)
        ...
        # gt_num x topk
        topk_inds = torch.topk(euclidean_dist1, seed_points_obj_topk, largest=False)[1] * box_label_mask[:, None] + (
        ...
        # pts_instance_label
        objectness_label_mask = torch.gather(pts_instance_label, 0, seed_indices)  # num_seed
        sampling_targets[objectness_label_mask < 0] = 0

        # 2. objectness target
        seed_obj_gt = torch.gather(pts_obj_mask, 0, seed_indices)  # num_seed
        objectness_targets = torch.gather(seed_obj_gt, 0, candidate_indices)  # num_candidate

        # 3. box target
        seed_instance_label = torch.gather(pts_instance_label, 0, seed_indices)  # num_seed
        query_points_instance_label = torch.gather(seed_instance_label, 0, candidate_indices)  # num_candidate
