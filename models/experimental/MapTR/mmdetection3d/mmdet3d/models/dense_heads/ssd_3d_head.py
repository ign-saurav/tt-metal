...
class SSD3DHead(VoteHead):
    ...
    def loss(
        ...
        centerness_loss = self.objectness_loss(
            bbox_preds["obj_scores"].transpose(2, 1), centerness_targets, weight=centerness_weights
        )

        # calculate center loss
        center_loss = self.center_loss(
            bbox_preds["center_offset"], center_targets, weight=box_loss_weights.unsqueeze(-1)
        )

        # calculate direction class loss
        dir_class_loss = self.dir_class_loss(
            bbox_preds["dir_class"].transpose(1, 2), dir_class_targets, weight=box_loss_weights
        )
        ...
        corner_loss = self.corner_loss(
            pred_corners3d, corner3d_targets.reshape(-1, 8, 3), weight=box_loss_weights.view(-1, 1, 1)
        )

        # calculate vote loss
        vote_loss = self.vote_loss(
            bbox_preds["vote_offset"].transpose(1, 2), vote_targets, weight=vote_mask.unsqueeze(-1)
        )
    ...
    def get_targets_single(
        ...
        (center_targets, size_targets, dir_class_targets, dir_res_targets) = self.bbox_coder.encode(
        ...
        if self.bbox_coder.with_rot:
            ...
            canonical_xyz = rotation_3d_in_axis(
                canonical_xyz.unsqueeze(0).transpose(0, 1), -gt_bboxes_3d.yaw[assignment], 2
            ).squeeze(1)
    ...
    def get_bboxes(self, points, bbox_preds, input_metas, rescale=False):
        ...
        for b in range(batch_size):
            ...
            bbox = input_metas[b]["box_type_3d"](
                bbox_selected.clone(), box_dim=bbox_selected.shape[-1], with_yaw=self.bbox_coder.with_rot
            )
    ...
    def multiclass_nms_single(self, obj_scores, sem_scores, bbox, points, input_meta):
        ...
        bbox = input_meta["box_type_3d"](
            bbox.clone(), box_dim=bbox.shape[-1], with_yaw=self.bbox_coder.with_rot, origin=(0.5, 0.5, 0.5)
        )
