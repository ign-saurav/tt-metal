...
class AnchorTrainMixin(object):
    ...
    def anchor_target_3d_single(
        ...
        if isinstance(self.bbox_assigner, list) and (not isinstance(anchors, list)):
            ...
            return self.anchor_target_single_assigner(
                self.bbox_assigner, anchors, gt_bboxes, gt_bboxes_ignore, gt_labels, input_meta, num_classes, sampling
            )
    ...
    def anchor_target_single_assigner(
        ...
        if len(gt_bboxes) > 0:
            ...
            pos_inds = (
                torch.nonzero(anchors.new_zeros((anchors.shape[0],), dtype=torch.bool) > 0, as_tuple=False)
                .squeeze(-1)
                .unique()
            )
            neg_inds = (
        ...
        if len(pos_inds) > 0:
            ...
            pos_dir_targets = get_direction_target(
                sampling_result.pos_bboxes, pos_bbox_targets, self.dir_offset, one_hot=False
            )
...
def get_direction_target(anchors, reg_targets, dir_offset=0, num_bins=2, one_hot=True):
    ...
    if one_hot:
        dir_targets = torch.zeros(
            *list(dir_cls_targets.shape), num_bins, dtype=anchors.dtype, device=dir_cls_targets.device
        )
