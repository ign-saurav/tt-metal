...
class FreeAnchor3DHead(Anchor3DHead):
    ...
    def loss(self, cls_scores, bbox_preds, dir_cls_preds, gt_bboxes, gt_labels, input_metas, gt_bboxes_ignore=None):
        ...
        num_pos = 0
        positive_losses = []
        for _, (anchors_, gt_labels_, gt_bboxes_, cls_prob_, bbox_preds_, dir_cls_preds_) in enumerate(
            ...
            with torch.no_grad():
                ...
                if indices.numel() == 0:
                    ...
                    image_box_prob = torch.sparse_coo_tensor(
                        indices.flip([0]), nonzero_box_prob, size=(anchors_.size(0), self.num_classes)
                    ).to_dense()
                # end
            ...
            if self.use_direction_classifier:
                ...
                matched_dir_targets = get_direction_target(
                    matched_anchors, matched_object_targets, self.dir_offset, one_hot=False
                )
                loss_dir = self.loss_dir(
                    dir_cls_preds_[matched].transpose(-2, -1), matched_dir_targets, reduction_override="none"
                )

            # generate bbox weights
            if self.diff_rad_by_sin:
                bbox_preds_[matched], matched_object_targets = self.add_sin_difference(
            ...
            loss_bbox = self.loss_bbox(
                bbox_preds_[matched], matched_object_targets, bbox_weights, reduction_override="none"
            ).sum(-1)
