...
namespace spconv {
template <typename Index, typename IndexGrid, unsigned NDim, int KernelMaxVolume = 256>
...
#pragma unroll
    ...
    for (int ix : tv::KernelLoopX<int>(numActIn)) {
        ...
        for (Index i = 0; i < numValidPoints; ++i) {
            ...
            index = tv::rowArrayIdx<Index, NDim>(pointPtr, outSpatialShape.data()) + spatialVolume * indicesIn(ix, 0);
...
#pragma unroll
    ...
    Index index = 0;
    for (int ix : tv::KernelLoopX<int>(numActIn)) {
        index = tv::rowArrayIdx<Index, NDim>(indicesIn.data() + ix * (NDim + 1) + 1, outSpatialShape.data()) +
...
template <typename Index, typename IndexGrid, unsigned NDim>
__global__ void resetGridKernel(const Index* indicePairUnique, tv::TensorView<IndexGrid> gridsOut, int numAct) {
    for (int ix : tv::KernelLoopX<int>(numAct)) {
