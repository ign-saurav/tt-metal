...
public:
    ...
    public:
        ...
        TV_HOST_DEVICE_INLINE bool operator==(const self_type& rhs) { return ptr_ == rhs.ptr_; }
        TV_HOST_DEVICE_INLINE bool operator!=(const self_type& rhs) { return ptr_ != rhs.ptr_; }
    ...
    TV_HOST_DEVICE_INLINE const_iterator begin() const { return const_iterator(mArray); }

    TV_HOST_DEVICE_INLINE const_iterator end() const { return const_iterator(mArray + mSize); }
    TV_HOST_DEVICE_INLINE const_iterator cbegin() const { return const_iterator(mArray); }

    TV_HOST_DEVICE_INLINE const_iterator cend() const { return const_iterator(mArray + mSize); }
...
bool operator==(const SimpleVector<T, MaxDim>& lfs, const SimpleVector<T, MaxDim>& rfs) {
...
struct ShapeBase : public SimpleVector<int, MaxDim> {
    ...
    TV_HOST_DEVICE_INLINE ShapeBase(std::initializer_list<int> shape) : SimpleVector<int, MaxDim>(shape) {}

    template <typename T, template <class...> class Container>
    ShapeBase(Container<T> shape) : SimpleVector<int, MaxDim>(shape) {}
    TV_HOST_DEVICE_INLINE ShapeBase(const ShapeBase<MaxDim>& shape) : SimpleVector<int, MaxDim>(shape) {}
...
template <class... Inds>
TV_HOST_DEVICE_INLINE unsigned rowArrayIdx(std::vector<int>& shape, Inds... indexes) {
    unsigned offset = 0;
    unsigned m = 1;
...
TV_HOST_DEVICE_INLINE unsigned rowArrayIdx(std::vector<int>& shape, std::vector<int>& indexes_vec) {
    unsigned offset = 0;
    unsigned m = 1;
...
template <class... Inds>
TV_HOST_DEVICE_INLINE unsigned rowArrayIdx(const Shape& shape, Inds... indexes) {
    unsigned offset = 0;
    unsigned m = 1;
    int indexes_vec[sizeof...(indexes)] = {indexes...};
#pragma unroll
...
TV_HOST_DEVICE_INLINE unsigned rowArrayIdx(const Shape& shape, const Shape& indexes_vec) {
    unsigned offset = 0;
    unsigned m = 1;
...
TV_HOST_DEVICE_INLINE unsigned rowArrayIdx(const Index* indexes, const Index* shape) {
    unsigned offset = 0;
    unsigned m = 1;
#pragma unroll
...
struct ArrayIndexRowMajor {
    ...
    TV_HOST_DEVICE_INLINE static unsigned run(const Shape& shape, const Shape& indexes) {
        return indexes[N - 1] + shape[N - 1] * ArrayIndexRowMajor<N - 1>::run(shape, indexes);
    }
};

template <>
struct ArrayIndexRowMajor<0> {
    TV_HOST_DEVICE_INLINE static unsigned run(const Shape& shape, const Shape& indexes) { return 0; }
};
...
struct TensorView {
    ...
    explicit TV_HOST_DEVICE_INLINE TensorView(T* ptr, Shape shape) : mPtr(ptr), mShape(shape) {}
    // explicit TV_HOST_DEVICE_INLINE TensorView(const
    // TensorView<std::remove_const_t<T>> &tview) : mPtr(tview.data()),
    // mShape(tview.shape()) {}
    template <class... Integers>
    explicit TV_HOST_DEVICE_INLINE TensorView(T* ptr, Integers... shapes) : mPtr(ptr) {
        mShape = {int(shapes)...};
    }

    TV_HOST_DEVICE_INLINE TensorView<T, Rank>& assign(const TensorView<T, Rank>& tensor) {
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank>& assign(std::initializer_list<T1> seq) {
    ...
    TV_HOST_DEVICE_INLINE T& operator()(Inds... inds) {
        ...
        TV_REQUIRE(
            sizeof...(inds) == mShape.ndim(),
            "you provide %d indexes, but dim is %d\n",
            sizeof...(inds),
            mShape.ndim());
        for (int i = 0; i < sizeof...(inds); ++i) {
            TV_REQUIRE(
                idxes[i] >= 0 && idxes[i] < mShape[i], "index-%d(%d) out-of-range: [0, %d)\n", i, idxes[i], mShape[i]);
        }
#endif
    ...
    TV_HOST_DEVICE_INLINE T& operator()() {
        ...
        TV_DEVICE_REQUIRE(mPtr != nullptr, "you want get value but the view is empty.%s", "\n");
        TV_DEVICE_REQUIRE(mShape.ndim() == 0, "you provide 0 indexes, but dim is %ld\n", mShape.ndim());
#else
...
#if defined TV_DEBUG
#if defined(__CUDA_ARCH__)
        TV_DEVICE_REQUIRE(mShape.ndim() == 1, "you provide 1 indexes, but dim is %ld\n", mShape.ndim());
        TV_DEVICE_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, i1, mShape[0]);
#else
        TV_REQUIRE(mShape.ndim() == 1, "you provide 1 indexes, but dim is %ld\n", mShape.ndim());
        TV_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, i1, mShape[0]);
#endif
#endif
        return mPtr[i1];
    }
...
#ifdef TV_DEBUG
#if defined(__CUDA_ARCH__)
        TV_DEVICE_REQUIRE(mShape.ndim() == 2, "you provide 2 indexes, but dim is %ld\n", mShape.ndim());
        TV_DEVICE_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_DEVICE_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
#else
        TV_REQUIRE(mShape.ndim() == 2, "you provide 2 indexes, but dim is %ld\n", mShape.ndim());
        TV_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
#endif
#endif
...
#ifdef TV_DEBUG
#if defined(__CUDA_ARCH__)
        TV_DEVICE_REQUIRE(mShape.ndim() == 3, "you provide 3 indexes, but dim is %ld\n", mShape.ndim());
        TV_DEVICE_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_DEVICE_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
        TV_DEVICE_REQUIRE(i3 >= 0 && i3 < mShape[2], "index-%d(%d) out-of-range: [0, %d)\n", 2, int(i3), mShape[2]);
#else
        TV_REQUIRE(mShape.ndim() == 3, "you provide 3 indexes, but dim is %ld\n", mShape.ndim());
        TV_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
        TV_REQUIRE(i3 >= 0 && i3 < mShape[2], "index-%d(%d) out-of-range: [0, %d)\n", 2, int(i3), mShape[2]);
#endif
#endif
...
#ifdef TV_DEBUG
#if defined(__CUDA_ARCH__)
        TV_DEVICE_REQUIRE(mShape.ndim() == 4, "you provide 4 indexes, but dim is %ld\n", mShape.ndim());
        TV_DEVICE_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_DEVICE_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
        TV_DEVICE_REQUIRE(i3 >= 0 && i3 < mShape[2], "index-%d(%d) out-of-range: [0, %d)\n", 2, int(i3), mShape[2]);
        TV_DEVICE_REQUIRE(i4 >= 0 && i4 < mShape[3], "index-%d(%d) out-of-range: [0, %d)\n", 3, int(i4), mShape[3]);
#else
        TV_REQUIRE(mShape.ndim() == 4, "you provide 4 indexes, but dim is %ld\n", mShape.ndim());
        TV_REQUIRE(i1 >= 0 && i1 < mShape[0], "index-%d(%d) out-of-range: [0, %d)\n", 0, int(i1), mShape[0]);
        TV_REQUIRE(i2 >= 0 && i2 < mShape[1], "index-%d(%d) out-of-range: [0, %d)\n", 1, int(i2), mShape[1]);
        TV_REQUIRE(i3 >= 0 && i3 < mShape[2], "index-%d(%d) out-of-range: [0, %d)\n", 2, int(i3), mShape[2]);
        TV_REQUIRE(i4 >= 0 && i4 < mShape[3], "index-%d(%d) out-of-range: [0, %d)\n", 3, int(i4), mShape[3]);
#endif
#endif
...
#ifdef TV_DEBUG
#if defined(__CUDA_ARCH__)
        TV_DEVICE_REQUIRE(idx >= 0 && idx < size(), "index(%d) out-of-range: [0, %ld)\n", int(idx), size());
#else
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank> operator[](SimpleVector<Slice> slice_vec) { return _subview(slice_vec); }
    TV_HOST_DEVICE_INLINE const TensorView<T, Rank> operator[](SimpleVector<Slice> slice_vec) const {
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank> squeeze() const { return TensorView<T, Rank>(mPtr, mShape.squeeze()); }
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank> subview(Slice slice, Slices... slices) const {
...
#pragma unroll
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank> subview(int id, Integers... ints) {
        ...
        return TensorView<T, Rank>(mPtr + rowArrayIdx(mShape, start), mShape.subshape(sizeof...(ints) + 1));
    }
...
protected:
    ...
    TV_HOST_DEVICE_INLINE TensorView<T, Rank> _subview(SimpleVector<Slice> slice_vec) {
        Shape new_shape;
...
template <typename T>
TV_HOST_DEVICE void printTensorView(const TensorView<T> tensor, const char* format) {
    if (tensor.empty()) {
        return;
    }
    if (tensor.ndim() == 0) {
...
TV_HOST_DEVICE void printTensorView(const T* ptr, Shape shape) {
    using Traw = typename std::remove_const<T>::type;
    return printTensorView(TensorView<const T>(ptr, shape), detail::printfTypeFormat<Traw>());
}
template <typename T>
TV_HOST_DEVICE void printTensorView(const T* ptr, Shape shape, const char* format) {
