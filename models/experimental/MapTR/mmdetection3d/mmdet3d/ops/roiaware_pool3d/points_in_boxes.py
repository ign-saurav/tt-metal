...
def points_in_boxes_gpu(points, boxes):
    ...
    assert boxes.shape[0] == points.shape[0], (
        f"Points and boxes should have the same batch size, " f"got {boxes.shape[0]} and {boxes.shape[0]}"
    )
    assert boxes.shape[2] == 7, f"boxes dimension should be 7, " f"got unexpected shape {boxes.shape[2]}"
    assert points.shape[2] == 3, f"points dimension should be 3, " f"got unexpected shape {points.shape[2]}"
...
def points_in_boxes_cpu(points, boxes):
    ...
    assert boxes.shape[1] == 7, f"boxes dimension should be 7, " f"got unexpected shape {boxes.shape[2]}"
    assert points.shape[1] == 3, f"points dimension should be 3, " f"got unexpected shape {points.shape[2]}"
