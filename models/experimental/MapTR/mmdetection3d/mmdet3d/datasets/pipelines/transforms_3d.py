...
class PointSample(object):
    ...
    def __call__(self, results):
        ...
        if self.sample_range is not None:
            ...
            assert isinstance(points, CameraPoints), "Sampling based on distance is only appliable for CAMERA coord"
        points, choices = self._points_random_sampling(
            points, self.num_points, self.sample_range, self.replace, return_choices=True
        )
...
class IndoorPatchPointSample(object):
    ...
    def _input_generation(self, coords, patch_center, coord_max, attributes, attribute_dims, point_type):
        ...
        if self.use_normalized_coord:
            ...
            attribute_dims.update(
                dict(normalized_coord=[attributes.shape[1], attributes.shape[1] + 1, attributes.shape[1] + 2])
            )
    ...
    def _patch_points_sampling(self, points, sem_mask):
        ...
        for _ in range(self.num_try):
            ...
            cur_choice = (
                np.sum((coords >= (cur_min - self.enlarge_size)) * (coords <= (cur_max + self.enlarge_size)), axis=1)
                == 3
            )
        ...
        points = self._input_generation(
            coords[choices], cur_center, coord_max, attributes[choices], attribute_dims, point_type
        )
...
class BackgroundPointsFilter(object):
    ...
    def __init__(self, bbox_enlarge_range):
        assert (is_tuple_of(bbox_enlarge_range, float) and len(bbox_enlarge_range) == 3) or isinstance(
    ...
    def _sample_points(self, points, sampler, point_dim):
        ...
        if voxels.shape[0] < sampler._max_voxels:
            padding_points = np.zeros(
                [sampler._max_voxels - voxels.shape[0], sampler._max_num_points, point_dim], dtype=points.dtype
            )
