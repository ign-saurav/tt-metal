...
class ScanNetSegDataset(Custom3DSegDataset):
    ...
    def _build_default_pipeline(self):
        ...
        pipeline = [
            ...
            dict(
                type="LoadAnnotations3D", with_bbox_3d=False, with_label_3d=False, with_mask_3d=False, with_seg_3d=True
            ),
            dict(
                type="PointSegClassMapping", valid_cat_ids=self.VALID_CLASS_IDS, max_cat_id=np.max(self.ALL_CLASS_IDS)
            ),
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            show_seg_result(
                points, gt_sem_mask, pred_sem_mask, out_dir, file_name, np.array(self.PALETTE), self.ignore_index, show
            )
