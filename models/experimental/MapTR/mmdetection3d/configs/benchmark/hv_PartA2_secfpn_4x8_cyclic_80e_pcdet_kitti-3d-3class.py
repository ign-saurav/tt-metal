...
model = dict(
    ...
    middle_encoder=dict(type="SparseUNet", in_channels=4, sparse_shape=[41, 1600, 1408], order=("conv", "norm", "act")),
    ...
    roi_head=dict(
        ...
        semantic_head=dict(
            ...
            seg_score_thr=0.3,
            num_classes=3,
            loss_seg=dict(type="FocalLoss", use_sigmoid=True, reduction="sum", gamma=2.0, alpha=0.25, loss_weight=1.0),
    ...
    train_cfg=dict(
        ...
        rpn_proposal=dict(nms_pre=9000, nms_post=512, max_num=512, nms_thr=0.8, score_thr=0, use_rotate_nms=False),
        rcnn=dict(
            assigner=[
                dict(  # for Pedestrian
                    type="MaxIoUAssigner",
    ...
    test_cfg=dict(
        rpn=dict(nms_pre=1024, nms_post=100, max_num=100, nms_thr=0.7, score_thr=0, use_rotate_nms=True),
        rcnn=dict(use_rotate_nms=True, use_raw_score=True, nms_thr=0.01, score_thr=0.3),
    ),
)
