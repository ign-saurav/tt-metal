...
model = dict(
    ...
    fusion_layer=dict(type="VoteFusion", num_classes=len(class_names), max_imvote_per_pixel=3),
    num_sampled_seed=1024,
    freeze_img_branch=True,
    # model training and testing settings
    train_cfg=dict(pts=dict(pos_distance_thr=0.3, neg_distance_thr=0.6, sample_mod="vote")),
    test_cfg=dict(
        img_rcnn=dict(score_thr=0.1), pts=dict(sample_mod="seed", nms_thr=0.25, score_thr=0.05, per_class_proposal=True)
    ),
)
