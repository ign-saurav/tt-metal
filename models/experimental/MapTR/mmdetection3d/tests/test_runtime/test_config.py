...
def _check_roi_extractor(config, roi_extractor, prev_roi_extractor=None):
    ...
    if prev_roi_extractor:
        ...
        assert roi_extractor.roi_layers[0].use_torchvision == prev_roi_extractor.roi_layers[0].use_torchvision
...
def _check_parta2_bbox_head(bbox_cfg, bbox_head):
    ...
    if isinstance(bbox_cfg, list):
        ...
        assert bbox_cfg.part_in_channels == bbox_head.part_conv[0][0].in_channels
...
def _check_h3d_bbox_head(bbox_cfg, bbox_head):
    ...
    assert bbox_cfg.suface_matching_cfg.mlp_channels[-1] * 18 == bbox_head.bbox_pred[0].in_channels
