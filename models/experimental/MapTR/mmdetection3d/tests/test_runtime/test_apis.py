...
def test_show_result_meshlab():
    ...
    box_3d = LiDARInstance3DBoxes(torch.tensor([[8.7314, -1.8559, -1.5997, 0.4800, 1.2000, 1.8900, 0.0100]]))
    ...
    box_3d = DepthInstance3DBoxes(torch.tensor([[-1.1580, 3.3041, -0.9961, 0.3829, 0.4647, 0.5574, 1.1213]]))
    img = np.random.randn(1, 3, 608, 832)
    k_mat = np.array([[529.5000, 0.0000, 365.0000], [0.0000, 529.5000, 265.0000], [0.0000, 0.0000, 1.0000]])
    rt_mat = np.array([[0.9980, 0.0058, -0.0634], [0.0058, 0.9835, 0.1808], [0.0634, -0.1808, 0.9815]])
    ...
    img_meta = dict(
        ...
        pcd_trans=np.array([0.0, 0.0, 0.0]),
    ...
    lidar2img = np.array(
        [
            [6.09695435e02, -7.21421631e02, -1.25125790e00, -1.23041824e02],
            [1.80384201e02, 7.64479828e00, -7.19651550e02, -1.01016693e02],
...
def test_inference_detector():
    ...
    pcd = "tests/data/kitti/training/velodyne_reduced/000000.bin"
    detector_cfg = "configs/pointpillars/hv_pointpillars_secfpn_" "6x8_160e_kitti-3d-3class.py"
...
def test_inference_multi_modality_detector():
    ...
    detector_cfg = "configs/imvotenet/imvotenet_stage2_" "16x8_sunrgbd-3d-10class.py"
    detector = init_model(detector_cfg, device="cuda:0")
    ...
    detector_cfg = "configs/mvxnet/dv_mvx-fpn_second_secfpn_adamw_" "2x8_80e_kitti-3d-3class.py"
    detector = init_model(detector_cfg, device="cuda:0")
    results = inference_multi_modality_detector(detector, pcd, img, ann_file)
...
def test_inference_mono_3d_detector():
    ...
    detector_cfg = "configs/fcos3d/fcos3d_r101_caffe_fpn_gn-head_dcn_" "2x8_1x_nus-mono3d.py"
    detector = init_model(detector_cfg, device="cuda:0")
...
def test_inference_segmentor():
    ...
    if not torch.cuda.is_available():
        pytest.skip("test requires GPU and torch+cuda")
    pcd = "tests/data/scannet/points/scene0000_00.bin"
    segmentor_cfg = "configs/pointnet2/pointnet2_ssg_" "16x2_cosine_200e_scannet_seg-3d-20class.py"
