...
def test_getitem():
    ...
    pipeline = [
        ...
        dict(
            ...
            db_sampler=dict(
                ...
                rate=1.0,
                prepare=dict(filter_by_difficulty=[-1], filter_by_min_points=dict(Pedestrian=10)),
                classes=["Pedestrian", "Cyclist", "Car"],
        ...
        dict(type="DefaultFormatBundle3D", class_names=["Pedestrian", "Cyclist", "Car"]),
    ...
    expected_gt_bboxes_3d = torch.tensor([[9.5081, -5.2269, -1.1370, 0.4915, 1.2288, 1.9353, -2.7136]])
    expected_gt_labels_3d = torch.tensor([0])
    assert points.shape == (780, 4)
    assert torch.allclose(gt_bboxes_3d.tensor, expected_gt_bboxes_3d, atol=1e-4)
    ...
    multi_modality_pipeline = [
        ...
        dict(type="Resize", img_scale=[(640, 192), (2560, 768)], multiscale_mode="range", keep_ratio=True),
        dict(
            type="GlobalRotScaleTrans",
            rot_range=[-0.78539816, 0.78539816],
    ...
    expected_lidar2img = np.array(
        [
            [6.02943726e02, -7.07913330e02, -1.22748432e01, -1.70942719e02],
            [1.76777252e02, 8.80879879e00, -7.07936157e02, -1.02568634e02],
...
def test_evaluate():
    ...
    kitti_dataset = KittiDataset(data_root, ann_file, split, pts_prefix, pipeline, classes, modality)
    boxes_3d = LiDARInstance3DBoxes(torch.tensor([[8.7314, -1.8559, -1.5997, 0.4800, 1.2000, 1.8900, 0.0100]]))
...
def test_bbox2result_kitti2d():
    ...
    bboxes = np.array(
        [
            [[46.1218, -4.6496, -0.9275, 0.5316, 0.5], [33.3189, 0.1981, 0.3136, 0.5656, 0.5]],
            [[46.1366, -4.6404, -0.9510, 0.5162, 0.5], [33.2646, 0.2297, 0.3446, 0.5746, 0.5]],
        ]
    )
