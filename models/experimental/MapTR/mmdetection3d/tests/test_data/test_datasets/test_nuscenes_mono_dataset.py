...
def test_getitem():
    ...
    expected_bboxes = torch.tensor(
        [
            [485.4207, 513.7568, 515.4637, 576.1393],
    ...
    expected_centers2d = torch.tensor(
        [[500.6090, 544.6358], [762.8789, 541.5280], [471.1633, 502.2295], [404.1957, 502.5908], [608.3627, 544.7317]]
    )
...
def test_format_results():
    ...
    assert torch.allclose(torch.tensor(det["translation"]), expected_trans, 1e-5)
    assert torch.allclose(torch.tensor(det["size"]), expected_size, 1e-5)
    assert torch.allclose(torch.tensor(det["rotation"]), expected_rotation, atol=1e-4)
...
def test_show():
    ...
    file_name = "n015-2018-07-18-11-07-57+0800__" "CAM_BACK_LEFT__1531883530447423"
