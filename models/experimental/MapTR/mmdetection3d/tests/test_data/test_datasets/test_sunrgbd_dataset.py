...
def _generate_sunrgbd_dataset_config():
    ...
    pipelines = [
        ...
        dict(
            ...
            meta_keys=["file_name", "pcd_horizontal_flip", "sample_idx", "pcd_scale_factor", "pcd_rotation"],
        ),
    ]
...
def _generate_sunrgbd_multi_modality_dataset_config():
    ...
    pipelines = [
        ...
        dict(type="PointSample", num_points=5),
        dict(type="DefaultFormatBundle3D", class_names=class_names),
        dict(type="Collect3D", keys=["img", "gt_bboxes", "gt_labels", "points", "gt_bboxes_3d", "gt_labels_3d"]),
    ]
...
def test_getitem():
    ...
    SUNRGBD_dataset = SUNRGBDDataset(root_path, ann_file, pipeline=None, classes=tmp_file.name)
    assert SUNRGBD_dataset.CLASSES != original_classes
    assert SUNRGBD_dataset.CLASSES == ["bed", "table"]
...
def test_show():
    ...
    scores_3d = torch.tensor([1.5280e-01, 1.6682e-03, 6.2811e-04, 1.2860e-03, 9.4229e-06])
