...
def test_getitem():
    ...
    pipeline = [
        ...
        dict(
            type="Collect3D",
            keys=["img", "gt_bboxes", "gt_labels", "gt_bboxes_3d", "gt_labels_3d", "centers2d", "depths"],
        ),
    ]
    ...
    expected_bboxes = torch.tensor(
        [
            [625.3445, 175.0120, 676.5177, 224.9605],
    ...
    expected_centers2d = torch.tensor(
        [[650.6185, 198.3731], [744.2711, 190.7532], [687.8787, 184.5331], [898.4750, 194.4337]]
    )
...
def test_format_results():
    ...
    kitti_dataset = KittiMonoDataset(
        ann_file=ann_file, info_file=info_file, pipeline=pipeline, data_root=root_path, test_mode=True
    )
    ...
    expected_bbox = torch.tensor(
        [
            [565.4989, 175.02547, 616.70184, 225.00565],
            [481.85907, 179.8642, 512.43414, 202.5624],
            [542.23157, 175.73912, 565.26263, 193.96303],
            [330.8572, 176.1482, 355.53937, 213.8469],
        ]
    )
    expected_dims = torch.tensor(
        [[3.201, 1.6110001, 1.661], [3.701, 1.401, 1.511], [4.051, 1.4610001, 1.661], [1.9510001, 1.7210001, 0.501]]
    )
    ...
    assert torch.allclose(torch.from_numpy(det["dimensions"]), expected_dims, 1e-5)
    assert torch.allclose(torch.from_numpy(det["rotation_y"]), expected_rotation, 1e-5)
    ...
    assert torch.allclose(torch.from_numpy(det["bbox"]).float(), expected_bbox, 1e-5)
    assert torch.allclose(torch.from_numpy(det["dimensions"]).float(), expected_dims, 1e-5)
    assert torch.allclose(torch.from_numpy(det["rotation_y"]).float(), expected_rotation, 1e-5)
    assert det["name"].tolist() == expected_detname
