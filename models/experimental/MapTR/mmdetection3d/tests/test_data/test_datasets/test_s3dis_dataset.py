...
def test_getitem():
    ...
    pipeline = [
        dict(type="LoadPointsFromFile", coord_type="DEPTH", shift_height=False, load_dim=6, use_dim=[0, 1, 2, 3, 4, 5]),
...
def test_seg_getitem():
    ...
    expected_points = torch.tensor(
        [
            ...
            [-0.4860, -0.0640, 3.1710, 0.3843, 0.3725, 0.3059, 0.3789, 0.7286, 0.9540],
        ]
    )
    ...
    s3dis_dataset = S3DISSegDataset(
        data_root=root_path, ann_files=ann_file, pipeline=None, classes=["beam", "window"], scene_idxs=scene_idxs
    )
    ...
    s3dis_dataset = S3DISSegDataset(
        data_root=root_path, ann_files=ann_file, pipeline=None, classes=tmp_file.name, scene_idxs=scene_idxs
    )
    assert s3dis_dataset.CLASSES != original_classes
    assert s3dis_dataset.CLASSES == ["beam", "window"]
    assert s3dis_dataset.PALETTE == [palette[3], palette[5]]
    ...
    # test mode
    s3dis_dataset = S3DISSegDataset(
        data_root=root_path, ann_files=ann_file, pipeline=None, test_mode=True, scene_idxs=scene_idxs
    )
...
def test_multi_areas():
    ...
    repeat_num = 3
    s3dis_dataset = S3DISSegDataset(
        data_root=root_path, ann_files=[ann_file for _ in range(repeat_num)], scene_idxs=scene_idxs
    )
    assert s3dis_dataset.CLASSES == class_names
