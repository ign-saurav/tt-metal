    ...
    db_sampler = mmcv.ConfigDict(
        {
            ...
            "rate": 1.0,
            "prepare": {"filter_by_difficulty": [-1], "filter_by_min_points": {"Pedestrian": 10}},
...
def test_object_noise():
    ...
    expected_gt_bboxes_3d = torch.tensor([[9.1724, -1.7559, -1.3550, 0.4800, 1.2000, 1.8900, 0.0505]])
    repr_str = repr(object_noise)
    expected_repr_str = (
        "ObjectNoise(num_try=100, "
...
def test_point_shuffle():
    ...
    ins_mask = np.fromfile("tests/data/scannet/instance_mask/scene0000_00.bin", np.long)
    sem_mask = np.fromfile("tests/data/scannet/semantic_mask/scene0000_00.bin", np.long)
...
def test_global_alignment():
    ...
    assert np.allclose(trans_depth_points.tensor.numpy(), expected_points, atol=1e-6)
...
def test_global_rot_scale_trans():
    ...
    global_rot_scale_trans = GlobalRotScaleTrans(
        rot_range=angle, scale_ratio_range=scale, translation_std=trans_std, shift_height=False
    )
    ...
    gt_bboxes_3d = DepthInstance3DBoxes(
        gt_bboxes_3d.copy(), box_dim=gt_bboxes_3d.shape[-1], with_yaw=False, origin=(0.5, 0.5, 0.5)
    )
    ...
    assert torch.allclose(trans_depth_points.tensor, true_depth_points.tensor, atol=1e-6)
    assert torch.allclose(trans_bboxes_3d.tensor, true_bboxes_3d.tensor, atol=1e-6)
    assert input_dict["pcd_scale_factor"] == scale_factor
    assert torch.allclose(input_dict["pcd_rotation"], noise_rot_mat_T, atol=1e-6)
    assert np.allclose(input_dict["pcd_trans"], trans_factor)

    repr_str = repr(global_rot_scale_trans)
    expected_repr_str = (
        f"GlobalRotScaleTrans(rot_range={[-angle, angle]},"
    ...
    global_rot_scale_trans = GlobalRotScaleTrans(
        rot_range=angle, scale_ratio_range=scale, translation_std=trans_std, shift_height=True
    )
    ...
    assert np.allclose(
        ...
        np.concatenate([true_depth_points.tensor.numpy(), true_shift_height], axis=1),
        atol=1e-6,
    )
...
def test_random_jitter_points():
    ...
    expected_repr_str = f"RandomJitterPoints(jitter_std={jitter_std}," f" clip_range={clip_range})"
    assert repr_str == expected_repr_str
    ...
    input_dict = random_jitter_points(input_dict)
    trans_depth_points = input_dict["points"]
    assert (trans_depth_points.tensor - depth_points.tensor).max().item() <= 0.05 + 1e-6
    assert (trans_depth_points.tensor - depth_points.tensor).min().item() >= -0.05 - 1e-6
...
def test_voxel_based_point_filter():
    np.random.seed(0)
    cur_sweep_cfg = dict(
        voxel_size=[0.1, 0.1, 0.1], point_cloud_range=[-50, -50, -4, 50, 50, 2], max_num_points=1, max_voxels=1024
    )
    prev_sweep_cfg = dict(
        voxel_size=[0.1, 0.1, 0.1], point_cloud_range=[-50, -50, -4, 50, 50, 2], max_num_points=1, max_voxels=1024
    )
    ...
    points = np.stack(
        [np.random.rand(4096) * 120 - 60, np.random.rand(4096) * 120 - 60, np.random.rand(4096) * 10 - 6], axis=-1
    )
...
def test_points_sample():
    ...
    select_idx = np.array(
        [622, 146, 231, 444, 504, 533, 80, 401, 379, 2, 707, 562, 176, 491, 496, 464, 15, 590, 194, 449]
    )
    expected_pts = points.tensor.numpy()[select_idx]
    assert np.allclose(sampled_pts.tensor.numpy(), expected_pts)

    repr_str = repr(point_sample)
    expected_repr_str = f"PointSample(num_points={num_points}, " f"sample_range={sample_range}, " "replace=False)"
    assert repr_str == expected_repr_str
