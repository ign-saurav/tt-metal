...
def test_voxel_net():
    if not torch.cuda.is_available():
        pytest.skip("test requires GPU and torch+cuda")
    _setup_seed(0)
    voxel_net_cfg = _get_detector_cfg("second/hv_second_secfpn_6x8_80e_kitti-3d-3class.py")
...
def test_vote_net():
    ...
    _setup_seed(0)
    vote_net_cfg = _get_detector_cfg("votenet/votenet_16x8_sunrgbd-3d-10class.py")
...
def test_parta2():
    if not torch.cuda.is_available():
        pytest.skip("test requires GPU and torch+cuda")
    _setup_seed(0)
    parta2 = _get_detector_cfg("parta2/hv_PartA2_secfpn_2x8_cyclic_80e_kitti-3d-3class.py")
...
def test_centerpoint():
    ...
    centerpoint = _get_detector_cfg(
        "centerpoint/centerpoint_0075voxel_second_secfpn_" "dcn_4x8_cyclic_flip-tta_20e_nus.py"
    )
    self = build_detector(centerpoint).cuda()
    points_0 = torch.rand([1000, 5], device="cuda")
    points_1 = torch.rand([1000, 5], device="cuda")
    points = [points_0, points_1]
    img_meta_0 = dict(box_type_3d=LiDARInstance3DBoxes, flip=True, pcd_horizontal_flip=True, pcd_vertical_flip=False)
    img_meta_1 = dict(box_type_3d=LiDARInstance3DBoxes, flip=True, pcd_horizontal_flip=False, pcd_vertical_flip=True)
    img_metas = [img_meta_0, img_meta_1]
    gt_bbox_0 = LiDARInstance3DBoxes(torch.rand([10, 9], device="cuda"), box_dim=9)
    gt_bbox_1 = LiDARInstance3DBoxes(torch.rand([10, 9], device="cuda"), box_dim=9)
    gt_bboxes = [gt_bbox_0, gt_bbox_1]
    gt_labels_0 = torch.randint(0, 3, [10], device="cuda")
    gt_labels_1 = torch.randint(0, 3, [10], device="cuda")
    gt_labels = [gt_labels_0, gt_labels_1]
...
def test_fcos3d():
    ...
    _setup_seed(0)
    fcos3d_cfg = _get_detector_cfg("fcos3d/fcos3d_r101_caffe_fpn_gn-head_dcn_2x8_1x_nus-mono3d.py")
    ...
    losses = self.forward_train(
        imgs, img_metas, gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths, attr_labels
    )
    assert losses["loss_cls"] >= 0
...
def test_groupfree3dnet():
    ...
    _setup_seed(0)
    groupfree3d_cfg = _get_detector_cfg("groupfree3d/groupfree3d_8x4_scannet-3d-18class-L6-O256.py")
    ...
    # test forward_train
    losses = self.forward_train(points, img_metas, gt_bboxes, gt_labels, pts_semantic_mask, pts_instance_mask)
...
def test_imvoxelnet():
    ...
    img_metas = [
        dict(
            ...
            lidar2img=np.array(
                [
                    [6.0e02, -7.2e02, -1.2e00, -1.2e02],
                    [1.8e02, 7.6e00, -7.1e02, -1.0e02],
