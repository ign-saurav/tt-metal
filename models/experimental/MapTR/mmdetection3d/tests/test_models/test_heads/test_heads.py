...
def test_anchor3d_head_loss():
    ...
    input_metas = [{"sample_idx": 1234}, {"sample_idx": 2345}]  # fake input_metas
...
def test_anchor3d_head_getboxes():
    ...
    # fake input_metas
    input_metas = [
        {"sample_idx": 1234, "box_type_3d": LiDARInstance3DBoxes, "box_mode_3d": Box3DMode.LIDAR},
        {"sample_idx": 2345, "box_type_3d": LiDARInstance3DBoxes, "box_mode_3d": Box3DMode.LIDAR},
    ]
    (cls_score, bbox_pred, dir_cls_preds) = self.forward(feats)
...
def test_part_aggregation_ROI_head():
    ...
    pred_scores = torch.tensor([0.9722, 0.7910, 0.4690, 0.3300, 0.3345], dtype=torch.float32, device="cuda")
    pred_labels = torch.tensor([0, 1, 0, 2, 1], dtype=torch.int64, device="cuda")
    pred_clses = torch.tensor(
        [
            [0.7874, 0.1344, 0.2190],
            [0.8193, 0.6969, 0.7304],
            [0.2328, 0.9028, 0.3900],
            [0.6177, 0.5012, 0.2330],
...
def test_free_anchor_3D_head():
    ...
    pts_bbox_head_cfg = _get_pts_bbox_head_cfg(
        "./free_anchor/hv_pointpillars_fpn_sbn-all_" "free-anchor_4x8_2x_nus-3d.py"
    )
...
def test_primitive_head():
    ...
    primitive_head_cfg = dict(
        ...
        conv_cfg=dict(type="Conv1d"),
        norm_cfg=dict(type="BN1d"),
        objectness_loss=dict(type="CrossEntropyLoss", class_weight=[0.4, 0.6], reduction="mean", loss_weight=1.0),
        center_loss=dict(type="ChamferDistance", mode="l1", reduction="sum", loss_src_weight=1.0, loss_dst_weight=1.0),
        semantic_reg_loss=dict(
            type="ChamferDistance", mode="l1", reduction="sum", loss_src_weight=1.0, loss_dst_weight=1.0
        ),
        semantic_cls_loss=dict(type="CrossEntropyLoss", reduction="sum", loss_weight=1.0),
        train_cfg=dict(
            dist_thresh=0.2, var_thresh=1e-2, lower_thresh=1e-6, num_point=100, num_point_line=10, line_thresh=0.2
        ),
    )
...
def test_dcn_center_head():
    ...
    dcn_center_head_cfg = dict(
        ...
        separate_head=dict(
            ...
            dcn_config=dict(
                type="DCN", in_channels=64, out_channels=64, kernel_size=3, padding=1, groups=4, bias=False
...
def test_fcos_mono3d_head():
    ...
    losses = self.loss(
        *ret_dict, gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths, attr_labels, img_metas
    )
    assert losses["loss_cls"] >= 0
