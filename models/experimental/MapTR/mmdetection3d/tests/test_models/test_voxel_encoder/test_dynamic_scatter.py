...
def test_dynamic_scatter():
    ...
    empty_feats_out_mean, empty_coors_out_mean = dsmean(empty_feats, empty_coors)
    ...
    empty_o_feats = torch.rand(size=(200000, 3), dtype=torch.float32, device="cuda") * 100 - 50
    empty_o_coors = torch.randint(low=-1, high=0, size=(200000, 3), dtype=torch.int32, device="cuda")

    empty_o_feats.requires_grad_()
    empty_o_feats_out_mean, empty_o_coors_out_mean = dsmean(empty_o_feats, empty_o_coors)
    empty_o_feats_out_mean.sum().backward()
    assert (empty_o_feats.grad == 0).all()

    empty_o_feats_out_max, empty_o_coors_out_max = dsmax(empty_o_feats, empty_o_coors)
    ...
    feats = torch.rand(size=(200000, 3), dtype=torch.float32, device="cuda") * 100 - 50
    coors = torch.randint(low=0, high=20, size=(200000, 3), dtype=torch.int32, device="cuda")
