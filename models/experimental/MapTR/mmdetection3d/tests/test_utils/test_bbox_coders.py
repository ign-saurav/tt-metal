...
def test_partial_bin_based_box_coder():
    ...
    center_target, size_class_target, size_res_target, dir_class_target, dir_res_target = box_coder.encode(
        gt_bboxes, gt_labels
    )
    expected_center_target = torch.tensor(
        [[0.8308, 4.1168, -0.2413], [2.3002, 4.8149, -0.7687], [-1.1477, 1.8090, -0.1444]]
    )
    expected_size_class_target = torch.tensor([0, 1, 2])
    expected_size_res_target = torch.tensor(
        [[0.1350, 0.2241, 0.9972], [-0.2193, -0.4166, 0.2328], [-0.2270, -0.3401, 1.2108]]
    )
    ...
    center = torch.tensor(
        [
            [
                [0.8014, 3.4134, -0.6133],
                [2.6375, 8.4191, 2.0438],
                [4.2017, 5.2504, -0.7851],
                [-1.0088, 5.4107, 1.6293],
    ...
    size_class = torch.tensor(
        [
            [
                [-1.0061, -2.2788, 1.1322, -4.4380, -11.0526, -2.8113, -2.0642, -7.5886, -4.8627, -5.0437],
                [-2.2058, -0.3527, -1.9976, 0.8815, -2.7980, -1.9053, -0.5097, -2.0232, -1.4242, -4.1192],
                [-1.4783, -0.1009, -1.1537, 0.3052, -4.3147, -2.6529, 0.2729, -0.3755, -2.6479, -3.7548],
                [-6.1809, -3.5024, -8.3273, 1.1252, -4.3315, -7.8288, -4.6091, -5.8153, 0.7480, -10.1396],
                [-9.0424, -3.7883, -6.0788, -1.8855, -10.2493, -9.7164, -1.0658, -4.1713, 1.1173, -10.6204],
            ]
        ]
    )
    ...
    size_res = torch.tensor(
        [
            [
                [
                    [-9.8976e-02, -5.2152e-01, -7.6421e-02],
                    ...
                    [1.8551e-01, 1.1308e00, -5.1864e-01],
                ...
                [
                    [-1.8725e00, -7.4102e-01, 1.0524e00],
                ...
                [
                    [-1.2576e00, -6.1550e-01, 7.9430e-01],
                    [-4.7222e-01, 1.5634e00, -5.9460e-02],
                    [-3.5367e-01, 1.3616e00, -1.6421e-01],
                    [-1.6611e-02, 2.4231e-01, -9.6188e-02],
                    [5.4486e-01, 4.6833e-01, 5.1151e-01],
                    [-6.1755e-01, 1.0292e00, 1.2458e00],
...
def test_anchor_free_box_coder():
    ...
    gt_bboxes = LiDARInstance3DBoxes(
        [
            [2.1227e00, 5.7951e00, -9.9900e-01, 1.6736e00, 4.2419e00, 1.5473e00, -1.5501e00],
            [1.1791e01, 9.0276e00, -8.5772e-01, 1.6210e00, 3.5367e00, 1.4841e00, -1.7369e00],
            [2.3638e01, 9.6997e00, -5.6713e-01, 1.7578e00, 4.6103e00, 1.5999e00, -1.4556e00],
        ]
    )
    ...
    expected_center_target = torch.tensor(
        [[2.1227, 5.7951, -0.2253], [11.7908, 9.0276, -0.1156], [23.6380, 9.6997, 0.2328]]
    )
    expected_size_targets = torch.tensor([[0.8368, 2.1210, 0.7736], [0.8105, 1.7683, 0.7421], [0.8789, 2.3052, 0.8000]])
    ...
    # test decode
    center = torch.tensor([[[14.5954, 6.3312, 0.7671], [67.5245, 22.4422, 1.5610], [47.7693, -6.7980, 1.4395]]])

    size_res = torch.tensor([[[-1.0752, 1.8760, 0.7715], [-0.8016, 1.1754, 0.0102], [-1.2789, 0.5948, 0.4728]]])

    dir_class = torch.tensor(
        [
            [
                [0.1512, 1.7914, -1.7658, 2.1572, -0.9215, 1.2139, 0.1749, 0.8606, 1.1743, -0.7679, -1.6005, 0.4623],
                [-0.3957, 1.2026, -1.2677, 1.3863, -0.5754, 1.7083, 0.2601, 0.1129, 0.7146, -0.1367, -1.2892, -0.0083],
                [-0.8862, 1.2050, -1.3881, 1.6604, -0.9087, 1.1907, -0.0280, 0.2027, 1.0644, -0.7205, -1.0738, 0.4748],
            ]
        ]
    )
    ...
    dir_res = torch.tensor(
        [
            [
                [1.1151, 0.5535, -0.2053, -0.6582, -0.1616, -0.1821, 0.4675, 0.6621, 0.8146, -0.0448, -0.7253, -0.7171],
                [0.7888, 0.2478, -0.1962, -0.7267, 0.0573, -0.2398, 0.6984, 0.5859, 0.7507, -0.1980, -0.6538, -0.6602],
                [0.9039, 0.6109, 0.1960, -0.5016, 0.0551, -0.4086, 0.3398, 0.2759, 0.7247, -0.0655, -0.5052, -0.9026],
            ]
        ]
    )
