...
def parse_args():
    ...
    parser.add_argument("--data-root", type=str, help="specify the root path of dataset")
    parser.add_argument("--nproc", type=int, default=64, required=False, help="workers to process data")
    args = parser.parse_args()
    return args


def create_av2_infos_mp(root_path, info_prefix, dest_path=None, split="train", num_multithread=64):
...
def get_data_from_logid(log_id, loader: AV2SensorDataLoader, data_root):
    ...
    for ts in cam_timestamps:
        cam_ring_fpath = [loader.get_closest_img_fpath(log_id, cam_name, ts) for cam_name in CAM_NAMES]
