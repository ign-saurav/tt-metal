...
CANDIDATE = ["n008-2018-08-01-15-16-36-0400__CAM_FRONT__1533151603512404.jpg"]
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        if "img" not in data:
            logger.error(f"\n no img in data for index {i}, available keys: {list(data.keys())}, skipping")
            continue

        if "img_metas" not in data:
            logger.error(f"\n no img_metas in data for index {i}, available keys: {list(data.keys())}, skipping")
            continue

        img = data["img"][0].data[0]
        img_metas = data["img_metas"][0].data[0]
        gt_bboxes_3d = data.get("gt_bboxes_3d", [None])[0] if "gt_bboxes_3d" in data else None
        gt_labels_3d = data.get("gt_labels_3d", [None])[0] if "gt_labels_3d" in data else None

        if len(img_metas) == 0:
            logger.error(f"\n empty img_metas for index {i}, skipping")
            continue
        ...
        # import pdb;pdb.set_trace()
        if len(CANDIDATE) > 0 and pts_filename not in CANDIDATE:
            continue
