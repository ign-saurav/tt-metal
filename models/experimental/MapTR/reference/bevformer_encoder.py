...
import copy
import warnings
from models.experimental.MapTR.reference.dependency import TRANSFORMER_LAYER, TRANSFORMER_LAYER_SEQUENCE
from models.experimental.MapTR.reference.dependency import TransformerLayerSequence
from models.experimental.MapTR.reference.dependency import force_fp32, auto_fp16
import numpy as np
import torch
from models.experimental.MapTR.reference.dependency import TORCH_VERSION, digit_version
...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def forward(
        ...
        **kwargs,
        ...
        shift_ref_2d = ref_2d.clone()
        # Handle broadcasting: ref_2d can be [bs, len_bev, num_bev_level, 2] or [D, len_bev, num_bev_level, 2]
        # shift shape can be [bs, 2] or [D, 2] or other, need to broadcast correctly
        if shift.dim() == 2:
            shift_size = shift.shape[0] * shift.shape[1]
            if ref_2d.dim() == 4:
                # Check if first dim matches batch size or is depth
                if ref_2d.shape[0] == shift.shape[0] and shift.shape[1] == 2:
                    # ref_2d is [bs, len_bev, num_bev_level, 2] and shift is [bs, 2]
                    shift = shift.view(shift.shape[0], 1, 1, 2)
                else:
                    # ref_2d is [D, len_bev, num_bev_level, 2] where D != bs (e.g., [18, 20000, 1, 2])
                    # shift might be [D, 2] or [bs, 2], expand to [D, 1, 1, 2] to match
                    D = ref_2d.shape[0]
                    if shift.shape[0] == D and shift.shape[1] == 2:
                        # shift is [D, 2]
                        shift = shift.view(D, 1, 1, 2)
                    elif shift.shape[0] * shift.shape[1] == 2:
                        # shift is [1, 2] or [2], expand to [D, 1, 1, 2]
                        shift = shift.view(-1, 2)[0:1].expand(D, -1).view(D, 1, 1, 2)
                    else:
                        # Take first element and expand
                        shift = shift.view(-1, 2)[0:1].expand(D, -1).view(D, 1, 1, 2)
            elif ref_2d.dim() == 5:
                # ref_2d is [D, len_bev, bs, num_bev_level, 2]
                D = ref_2d.shape[0]
                bs_ref = ref_2d.shape[2]
                if shift.shape[0] == bs_ref and shift.shape[1] == 2:
                    shift = shift.view(1, 1, bs_ref, 1, 2).expand(D, -1, -1, -1, -1)
                else:
                    # Take first element and expand
                    shift = shift[0:1].view(1, 1, 1, 1, 2).expand(D, -1, bs_ref, -1, -1)
            else:
                # Fallback: expand to match ref_2d dimensions
                shift = shift[0:1].view(*([1] * (ref_2d.dim() - 2)), 2).expand(*ref_2d.shape[:-1], -1)
        shift_ref_2d += shift
