...
from models.experimental.MapTR.dependency import is_tuple_of as mmcv_is_tuple_of
...
from models.experimental.MapTR.dependency import ConvModule
...
class SELayer(BaseModule):
    ...
    def __init__(
        self, channels, ratio=16, conv_cfg=None, act_cfg=(dict(type="ReLU"), dict(type="Sigmoid")), init_cfg=None
    ):
        super(SELayer, self).__init__(init_cfg)
        if isinstance(act_cfg, dict):
            act_cfg = (act_cfg, act_cfg)
        assert len(act_cfg) == 2
        assert mmcv_is_tuple_of(act_cfg, dict)
