...
# from models.experimental.MapTR.dependency import BboxOverlaps3D
...
def class_tp_curve(
    ...
    if ax is None:
        ax = setup_axis(
            title=PRETTY_DETECTION_NAMES[detection_name], xlabel="Recall", ylabel="Error", xlim=1, min_recall=min_recall
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate nuScenes detection results.", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to the configuration file." "If no path given, the CVPR 2019 configuration will be used.",
    )
    parser.add_argument(
        "--plot_examples", type=int, default=0, help="How many example visualizations to write to disk."
    )
    ...
    nusc_eval = NuScenesEval_custom(
        nusc_, config=cfg_, result_path=result_path_, eval_set=eval_set_, output_dir=output_dir_, verbose=verbose_
    )
    for vis in ["1", "2", "3", "4"]:
