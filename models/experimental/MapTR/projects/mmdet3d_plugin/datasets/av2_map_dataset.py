...
class LiDARInstanceLines(object):
    ...
    def shift_fixed_num_sampled_points(self):
        ...
        for fixed_num_pts in fixed_num_sampled_points:
            ...
            if not is_poly:
                padding = torch.full(
                    [fixed_num - shift_pts.shape[0], fixed_num, shift_pts.shape[-1]], self.padding_value
                )
    ...
    def shift_fixed_num_sampled_points_v4(self):
        ...
        for fixed_num_pts in fixed_num_sampled_points:
            ...
            if not is_poly:
                padding = torch.full(
                    [shift_num * 2 - shift_pts.shape[0], pts_num, shift_pts.shape[-1]], self.padding_value
                )
                shift_pts = torch.cat([shift_pts, padding], dim=0)
                # padding = np.zeros((self.num_samples - len(sampled_points), 2))
                # sampled_points = np.concatenate([sampled_points, padding], axis=0)
...
class VectorizedAV2LocalMap(object):
    CLASS2LABEL = {"road_divider": 0, "lane_divider": 0, "divider": 0, "ped_crossing": 1, "boundary": 2, "others": -1}
