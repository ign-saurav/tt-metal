...
from models.experimental.MapTR.dependency import TRANSFORMER
from models.experimental.MapTR.dependency import xavier_init
from models.experimental.MapTR.dependency import BaseModule
from models.experimental.MapTR.dependency import build_transformer_layer_sequence
...
from models.experimental.MapTR.projects.mmdet3d_plugin.bevformer.modules.decoder import CustomMSDeformableAttention
...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def attn_bev_encode(
        ...
        # Handle can_bus that might be shorter than expected
        ...
        # Convert numpy arrays to tensor and ensure correct shape [2, bs]
        ...
        # Permute from [2, bs] to [bs, 2]
        if shift.dim() == 2:
            shift = shift.permute(1, 0)  # xy, bs -> bs, xy
        else:
            # If somehow 3D, reshape first
        ...
        # add can bus signals
        # Convert can_bus to tensor, handling both list and array formats
        can_bus_list = []
        for each in kwargs["img_metas"]:
            cb = each.get("can_bus", [0.0] * self.len_can_bus)
            # Ensure can_bus is a list/array and pad/truncate to len_can_bus
            if isinstance(cb, np.ndarray):
                ...
                cb = list(cb)
            # Pad or truncate to len_can_bus
        ...
        # Convert to numpy array first, then to tensor to avoid scalar conversion issues
        can_bus_array = np.array(can_bus_list, dtype=np.float32)
        can_bus = torch.from_numpy(can_bus_array).to(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if feat.shape[-1] != self.embed_dims:
                if self.feat_proj is None or (
                    ...
                    # Initialize or recreate projection layer
