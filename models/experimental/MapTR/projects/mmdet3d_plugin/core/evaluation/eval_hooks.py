...
from models.experimental.MapTR.dependency import is_list_of
import torch.distributed as dist
from models.experimental.MapTR.dependency import BaseDistEvalHook
...
class CustomDistEvalHook(BaseDistEvalHook):
    ...
    def _do_evaluate(self, runner):
        ...
        # Training evaluation hooks removed for inference-only build
        # from projects.mmdet3d_plugin.bevformer.apis.test import custom_multi_gpu_test  # to solve circlur  import

        results = custom_multi_gpu_test(runner.model, self.dataloader, tmpdir=tmpdir, gpu_collect=self.gpu_collect)
        if runner.rank == 0:
            print("\n")
