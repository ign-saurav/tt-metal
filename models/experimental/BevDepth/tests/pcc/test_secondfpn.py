...
def test_secondfpn_pcc(device, batch_size, height, width):
    ...
    torch_layer1 = torch.randn(batch_size, 256, target_h * 4, target_w * 4)
    torch_layer2 = torch.randn(batch_size, 512, target_h * 2, target_w * 2)
    torch_layer3 = torch.randn(batch_size, 1024, target_h, target_w)
    torch_layer4 = torch.randn(batch_size, 2048, target_h // 2, target_w // 2)
    ...
    for i in range(4):
        ...
        if hasattr(bn_layer, "weight") and hasattr(bn_layer, "running_mean"):
            ...
            if is_transposed:
                conv_out_channels = conv_weight.shape[1]
    ...
    assert pcc_value > 0.97, f"SECONDFPN PCC {pcc_value:.6f} is below threshold 0.97"
