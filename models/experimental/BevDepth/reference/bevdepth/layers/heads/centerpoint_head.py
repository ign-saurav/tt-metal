...
def nms_bev(
    ...
    # Fix Me: nms_rotated is not called due to compatibility issues with mmcv.ops.nms_rotated.
    keep = []
    keep = order[keep]
...
class DCNSeparateHead(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        cls_head = [
            ConvModule(
                in_channels, head_conv, kernel_size=3, padding=1, conv_cfg=conv_cfg, bias=bias, norm_cfg=norm_cfg
            ),
            build_conv_layer(conv_cfg, head_conv, num_cls, kernel_size=3, stride=1, padding=1, bias=bias),
        ]
...
class CenterHead(BaseModule):
        ...
        Returns:
            Returns:
                ...
                - list[torch.Tensor]: Masks indicating which
                    boxes are valid.
        """
        heatmaps, anno_boxes, inds, masks = multi_apply(self.get_targets_single, batch_gt_instances_3d)
        # Transpose heatmaps
    ...
    def predict_by_feat(
        self, preds_dicts: Tuple[List[dict]], batch_input_metas: List[dict], *args, **kwargs
    ) -> List[InstanceData]:
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            temp = self.bbox_coder.decode(
                batch_heatmap, batch_rots, batch_rotc, batch_hei, batch_dim, batch_vel, reg=batch_reg, task_id=task_id
            )
            ...
            if self.test_cfg["nms_type"] == "circle":
                ...
                rets.append(
                    self.get_task_detections(
                        num_class_with_bg, batch_cls_preds, batch_reg_preds, batch_cls_labels, batch_input_metas
                    )
                )
            ...
            if selected_boxes.shape[0] != 0:
                ...
                if post_center_range is not None:
                    ...
                    predictions_dict = dict(
                        bboxes=final_box_preds[mask], scores=final_scores[mask], labels=final_labels[mask]
                    )
                else:
