...
def draw_heatmap_gaussian(heatmap: Tensor, center: Tensor, radius: int, k: int = 1) -> Tensor:
    ...
    masked_gaussian = torch.from_numpy(gaussian[radius - top : radius + bottom, radius - left : radius + right]).to(
        heatmap.device, torch.float32
    )
...
def get_ellip_gaussian_2D(heatmap: Tensor, center: List[int], radius_x: int, radius_y: int, k: int = 1) -> Tensor:
    ...
    masked_heatmap = heatmap[y - top : y + bottom, x - left : x + right]
    masked_gaussian = gaussian_kernel[radius_y - top : radius_y + bottom, radius_x - left : radius_x + right]
    out_heatmap = heatmap
...
def ellip_gaussian2D(
    radius: Tuple[int, int], sigma_x: int, sigma_y: int, dtype: torch.dtype = torch.float32, device: str = "cpu"
) -> Tensor:
