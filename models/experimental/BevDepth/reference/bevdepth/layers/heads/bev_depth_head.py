...
class BEVDepthHead(CenterHead):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for idx, task_head in enumerate(self.task_heads):
            heatmap = gt_bboxes_3d.new_zeros(
                (len(self.class_names[idx]), feature_map_size[1], feature_map_size[0]), device="cuda"
            )

            anno_box = gt_bboxes_3d.new_zeros(
                (max_objs, len(self.train_cfg["code_weights"])), dtype=torch.float32, device="cuda"
            )
    ...
    def loss(self, targets, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if "vel" in preds_dict[0].keys():
                ...
                preds_dict[0]["anno_box"] = torch.cat(
                    (preds_dict[0]["reg"], preds_dict[0]["height"], preds_dict[0]["dim"], preds_dict[0]["rot"]),
                    dim=1,
                )
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            assert self.test_cfg["nms_type"] in ["size_aware_circle", "circle", "rotate"]
            ...
            if self.test_cfg["nms_type"] == "circle":
                ...
                rets.append(
                    self.get_task_detections(
                        num_class_with_bg, batch_cls_preds, batch_reg_preds, batch_cls_labels, img_metas
                    )
                )
