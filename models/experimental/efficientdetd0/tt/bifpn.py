...
class TtBiFPN:
    ...
    def _pre_upsample_reshape(self, x, config: UpsampleConfiguration):
        ...
        x = self._reshape(x, unflattened_shape)
        return x
    ...
    def _forward_fast_attention(self, inputs: Tuple[ttnn.Tensor, ...]) -> Tuple[ttnn.Tensor, ...]:
        ...
        if self.first_time:
            ...
            # Convert to interleaved if sharded since maxpool will need to reshard
            if p6_in.is_sharded():
                p6_in = ttnn.sharded_to_interleaved(p6_in, ttnn.L1_MEMORY_CONFIG)
        ...
        p6_in = self._reshape(p6_in, p7_upsampled.shape)
        ...
        p5_in = self._reshape(p5_in, p6_upsampled.shape)
        ...
        p4_in = self._reshape(p4_in, p5_upsampled.shape)
        ...
        p3_in = self._reshape(p3_in, p4_upsampled.shape)
        ...
        # Convert to interleaved if sharded since maxpool will need to reshard
        if p3_out.is_sharded():
            p3_out = ttnn.sharded_to_interleaved(p3_out, ttnn.L1_MEMORY_CONFIG)
        p3_downsampled = self.p4_downsample(p3_out)
        p4_up = self._reshape(p4_up, p4_in.shape)
        p3_downsampled = self._reshape(p3_downsampled, p4_in.shape)
        ...
        # Convert to interleaved if sharded since maxpool will need to reshard
        if p4_out.is_sharded():
            p4_out = ttnn.sharded_to_interleaved(p4_out, ttnn.L1_MEMORY_CONFIG)
        p4_downsampled = self.p5_downsample(p4_out)
        p5_up = self._reshape(p5_up, p5_in.shape)
        p4_downsampled = self._reshape(p4_downsampled, p5_in.shape)
        ...
        # Convert to interleaved if sharded since maxpool will need to reshard
        if p5_out.is_sharded():
            p5_out = ttnn.sharded_to_interleaved(p5_out, ttnn.L1_MEMORY_CONFIG)
        p5_downsampled = self.p6_downsample(p5_out)
        p6_up = self._reshape(p6_up, p6_in.shape)
        p5_downsampled = self._reshape(p5_downsampled, p6_in.shape)
        ...
        # Convert to interleaved if sharded since maxpool will need to reshard
        if p6_out.is_sharded():
            p6_out = ttnn.sharded_to_interleaved(p6_out, ttnn.L1_MEMORY_CONFIG)
        p6_downsampled = self.p7_downsample(p6_out)
        p6_downsampled = self._reshape(p6_downsampled, p7_in.shape)
