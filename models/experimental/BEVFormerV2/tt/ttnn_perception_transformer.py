...
from models.experimental.BEVFormerV2.tt.ttnn_decoder import TtDecoder, TtMapDecoder
...
class TtPerceptionTransformer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(TtPerceptionTransformer, self).__init__(**kwargs)
    ...
    def attn_bev_encode(
        ...
        **kwargs,
        ...
        if self.use_can_bus:
            ...
            bev_queries = ttnn.to_layout(bev_queries, layout=ttnn.TILE_LAYOUT)
            bev_queries = bev_queries + can_bus
            bev_queries = ttnn.to_layout(bev_queries, layout=ttnn.ROW_MAJOR_LAYOUT)
        ...
        for lvl in range(num_levels_to_use):
            feat = mlvl_feats[lvl]
            feat = ttnn.to_layout(feat, layout=ttnn.ROW_MAJOR_LAYOUT)
            ...
            if self.use_cams_embeds:
                ...
                cam_embeds = ttnn.to_layout(cam_embeds, layout=ttnn.ROW_MAJOR_LAYOUT)
                feat = feat + cam_embeds
                ttnn.deallocate(cam_embeds)
            level_embeds = params.level_embeds
            level_idx = min(lvl, params.level_embeds.shape[0] - 1)
            level_embeds = level_embeds[level_idx : level_idx + 1, :]
            level_embeds = ttnn.reshape(level_embeds, (1, 1, level_embeds.shape[0], level_embeds.shape[-1]))
            level_embeds = ttnn.to_layout(level_embeds, layout=ttnn.ROW_MAJOR_LAYOUT)
