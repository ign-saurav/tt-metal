...
def create_transfuser_pipeline_model(ttnn_model, tt_image, tt_lidar_bev, tt_velocity, target_point):
    ...
    def run(tt_lidar_bev_pipeline):
        if tt_lidar_bev_pipeline.memory_config().buffer_type == ttnn.BufferType.L1:
            tt_lidar_bev_pipeline_dram = ttnn.to_memory_config(tt_lidar_bev_pipeline, ttnn.DRAM_MEMORY_CONFIG)
            ttnn.deallocate(tt_lidar_bev_pipeline)
