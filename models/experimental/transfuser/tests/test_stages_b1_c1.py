...
def keep_only_layer1_b1_conv1(model):
    """
    Modifies the given model in-place to keep only 'layer1.b1.conv1' parameters
    in its state_dict. All other parameters are removed.

    Args:
        model (torch.nn.Module): The PyTorch model to modify

    Returns:
        torch.nn.Module: The same model, now containing only 'layer1.b1.conv1' params
    """
    # Get full state_dict
    full_state_dict = model.state_dict()

    # Filter keys
    filtered_state_dict = OrderedDict({k: v for k, v in full_state_dict.items() if "layer1.b1.conv1" in k})

    # Load filtered dict back into the model (non-strict to allow missing keys)
    model.load_state_dict(filtered_state_dict, strict=False)

    print("✅ Model updated: only 'layer1.b1.conv1' layers retained.")
...
class StageInfra:
    def __init__(
        ...
        # torch_model = keep_only_layer1_b1_conv1(torch_model)
        ...
        onnx_filename = f"stage_{stage_name}_s1_b1.onnx"
        # try:
        ...
        stage_to_pt_file = {
            "layer1": "image_features_layer1_original.pt",
    ...
    def validate(self, model_config, output_tensor=None):
        ...
        tt_tensor_torch = self.torch_model.forward_c3(tt_tensor_torch)
