...
def read_calib(file_path, extend_matrix=True):
    ...
    calib_dict = dict(
        P0=P0, P1=P1, P2=P2, P3=P3, R0_rect=R0_rect, Tr_velo_to_cam=Tr_velo_to_cam, Tr_imu_to_velo=Tr_imu_to_velo
    )
    return calib_dict
...
def write_label(result, file_path, suffix=".txt"):
    ...
    with open(file_path, "w") as f:
        for i in range(len(name)):
            ...
            line = (
                f"{name[i]} {truncated[i]} {occluded[i]} {alpha[i]} {bbox_str} {hwl} {xyz} {rotation_y[i]} {score[i]}\n"
            )
            f.writelines(line)
