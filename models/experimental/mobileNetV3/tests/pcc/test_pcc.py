...
def test_invertedResidual(
    ...
    for idx, (tt_block, torch_layer) in enumerate(zip(ttnn_blocks, torch_model)):
        ...
        # PCC check
        try:
            passed, msg = check_with_pcc(tt_as_torch, x_torch, pcc=0.99)
            if passed:
    ...
    if include_classifier:
        ...
        x_tt = ttnn.to_layout(x_tt, layout=ttnn.ROW_MAJOR_LAYOUT)
        x_tt = ttnn.reshape(x_tt, (x_tt.shape[0], -1))
        x_tt = ttnn.to_layout(x_tt, layout=ttnn.TILE_LAYOUT)
        idx += 1

        tt_as_torch = ttnn.to_torch(x_tt)
        tt_as_torch = torch.permute(tt_as_torch, (0, 1))
        try:
            passed, msg = check_with_pcc(tt_as_torch, x_torch, pcc=0.99)
            if passed:
    ...
    try:
        passed, msg = check_with_pcc(x_torch_og, x_torch, pcc=0.99)
