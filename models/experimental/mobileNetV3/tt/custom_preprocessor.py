...
def linear_to_params(weight, bias):
...
def se_to_params(se):
    ...
    return {
        "fc1": {
            "weight": ttnn.from_torch(se.fc1.weight, dtype=ttnn.bfloat16),
            "bias": ttnn.from_torch(torch.reshape(se.fc1.bias, (1, 1, 1, -1)), dtype=ttnn.bfloat16),
        },
        "fc2": {
            "weight": ttnn.from_torch(se.fc2.weight, dtype=ttnn.bfloat16),
            "bias": ttnn.from_torch(torch.reshape(se.fc2.bias, (1, 1, 1, -1)), dtype=ttnn.bfloat16),
        },
    }
        ...
        if isinstance(model, MobileNetV3):
            ...
            for idx, child in enumerate(model.features.children()):
                if isinstance(child, Conv2dNormActivation):
                    features[idx] = {0: conv_bn_to_params(child[0], child[1])}
                elif isinstance(child, InvertedResidual):
                    features[idx] = {"block": custom_preprocessor(child, name, ttnn_module_args)}
            parameters["features"] = features

            parameters["classifier"] = {
                0: linear_to_params(model.classifier[0].weight, model.classifier[0].bias),
                3: linear_to_params(model.classifier[3].weight, model.classifier[3].bias),
            }
        ...
        # Case 2: InvertedResidual
            ...
            for idx, child in enumerate(model.block.children()):
                if isinstance(child, SElayer):
                    parameters[idx] = se_to_params(child)
                elif isinstance(child, Conv2dNormActivation):
                    parameters[idx] = {0: conv_bn_to_params(child[0], child[1])}

        # Case 3: SElayer
        elif isinstance(model, SElayer):
            parameters = se_to_params(model)
