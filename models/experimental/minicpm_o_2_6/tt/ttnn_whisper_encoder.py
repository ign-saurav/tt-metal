...
class TtnnWhisperEncoder:
    ...
    def __init__(
        ...
        # Note: Weights are loaded via load_weights() method after initialization
            ...
            layer_params = {
                "self_attn": {
                    "query": {
                        ...
                        "bias": weights_dict.get(
                            f"layers.{layer_idx}.self_attn.q_proj.bias",
                            torch.zeros(weights_dict[f"layers.{layer_idx}.self_attn.q_proj.weight"].shape[0]),
                        ),
                    },
                    "key": {
                        ...
                        "bias": weights_dict.get(
                            ...
                            torch.zeros(weights_dict[f"layers.{layer_idx}.self_attn.k_proj.weight"].shape[0]),
                        ),
                    },
                    "value": {
                        ...
                        "bias": weights_dict.get(
                            ...
                            torch.zeros(weights_dict[f"layers.{layer_idx}.self_attn.v_proj.weight"].shape[0]),
                        ),
                    },
                    "out_proj": {
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask: Optional attention mask of shape (batch_size, 1, tgt_len, src_len)
            head_mask: Optional head mask of shape (encoder_layers, encoder_attention_heads)
...
def preprocess_encoder_inputs_minicpm(config, input_features, *, parameters, device):
    ...
    # Get conv configs for conv2d without fused activation (apply GELU separately)
    ...
    # Conv1 without fused activation
    ...
    conv2_weight = ttnn.reshape(parameters["conv2"]["weight"], shape=[config.d_model, config.d_model, 3, 1])

    conv2_weight = ttnn.to_device(conv2_weight, device, memory_config=ttnn.DRAM_MEMORY_CONFIG)
    ...
    # Conv2 without fused activation
    conv2_output = ttnn.conv2d(
        input_tensor=conv1_output,
...
def encoder_minicpm(config, inputs_embeds, *, parameters, attention_mask=None, head_mask=None):
    """
    ...
    # Validate head_mask shape if provided
    if head_mask is not None:
        if head_mask.shape[0] != config.encoder_layers:
            raise ValueError(
                f"head_mask should be specified for {config.encoder_layers} layers, "
                f"but is for {head_mask.shape[0]} layers."
            )
    ...
    for layer_idx in range(config.encoder_layers):
        ...
        hidden_states = encoder_layer_minicpm(
            config,
            hidden_states,
...
def encoder_layer_minicpm(
    ...
    *,
    ...
    Args:
        ...
        device: TTNN device
        attention_mask: Optional attention mask of shape (batch_size, 1, tgt_len, src_len)
        layer_head_mask: Optional head mask for this layer of shape (encoder_attention_heads,)
    ...
    # MLP with HiFi4 precision - separate GELU to match PyTorch exactly
    hifi4_config = ttnn.WormholeComputeKernelConfig(
        math_fidelity=ttnn.MathFidelity.HiFi4,
        math_approx_mode=False,
        fp32_dest_acc_en=True,  # This enables FP32 accumulation
        packer_l1_acc=True,
    )
...
def whisper_attention_minicpm(
    ...
    *,
    ...
    # Linear transformations for Q, K, V with HiFi4 precision
    hifi4_config = ttnn.WormholeComputeKernelConfig(
        math_fidelity=ttnn.MathFidelity.HiFi4,
        math_approx_mode=False,
        fp32_dest_acc_en=True,  # This enables FP32 accumulation
    ...
    # Reshape for attention (head_size already computed above)
    ...
    # Compute attention: Q * K^T with high precision
    ...
    # Apply attention_mask if provided (add to attention weights before softmax)
    if attention_mask is not None:
        # attention_mask shape: (batch_size, 1, tgt_len, src_len) or (batch_size, 1, 1, src_len)
        # attn_weights shape: (bsz, num_heads, seq_len, seq_len)
        # Need to expand attention_mask to match attn_weights shape
        device = hidden_states.device()

        # Convert attention_mask to TTNN tensor if it's a torch tensor
        if isinstance(attention_mask, torch.Tensor):
            # Get actual mask dimensions
            mask_bsz, mask_heads, mask_tgt_len, mask_src_len = attention_mask.shape

            # Expand attention_mask: (bsz, 1, tgt_len, src_len) -> (bsz, num_heads, tgt_len, src_len)
            # If mask has shape (bsz, 1, 1, src_len), expand to (bsz, 1, tgt_len, src_len) first
            if mask_tgt_len == 1:
                attention_mask_expanded = attention_mask.expand(bsz, 1, tgt_len, mask_src_len)
            else:
                attention_mask_expanded = attention_mask

            # Ensure mask dimensions match attn_weights (for self-attention, tgt_len == src_len == seq_len)
            # If mask_src_len doesn't match tgt_len, we need to handle it (shouldn't happen in self-attention)
            final_tgt_len = tgt_len if mask_tgt_len == 1 else mask_tgt_len
            final_src_len = mask_src_len if mask_src_len == tgt_len else tgt_len

            if mask_src_len != tgt_len and mask_tgt_len != 1:
                logger.warning(
                    f"Attention mask src_len ({mask_src_len}) != tgt_len ({tgt_len}), using tgt_len for both dimensions"
                )
                final_src_len = tgt_len

            # Now expand to match number of heads: (bsz, 1, tgt_len, src_len) -> (bsz, num_heads, tgt_len, src_len)
            attention_mask_expanded = attention_mask_expanded.expand(
                bsz, config.encoder_attention_heads, final_tgt_len, final_src_len
            )

            attention_mask_ttnn = ttnn.from_torch(
                attention_mask_expanded.to(dtype=torch.bfloat16),
                dtype=ttnn.bfloat16,
                layout=ttnn.TILE_LAYOUT,
                device=device,
            )
        else:
            attention_mask_ttnn = attention_mask

        # Add attention mask to attention weights (masked positions should be -inf, which becomes 0 after softmax)
        attn_weights = ttnn.add(attn_weights, attention_mask_ttnn)

    # Apply softmax with HiFi4 precision and FP32 accumulation for numerical stability
    attn_weights = ttnn.softmax(
        ...
        numeric_stable=True,  # Ensure numerical stability
    )

    # Apply layer_head_mask if provided (multiply with attention weights after softmax)
    if layer_head_mask is not None:
        # layer_head_mask shape: (encoder_attention_heads,)
        # attn_weights shape: (bsz, num_heads, seq_len, seq_len)
        # Need to reshape layer_head_mask to (1, num_heads, 1, 1) for broadcasting
        device = hidden_states.device()

        # Convert layer_head_mask to TTNN tensor if it's a torch tensor
        if isinstance(layer_head_mask, torch.Tensor):
            # Reshape to (1, num_heads, 1, 1) for broadcasting
            layer_head_mask_reshaped = layer_head_mask.view(1, config.encoder_attention_heads, 1, 1)
            layer_head_mask_ttnn = ttnn.from_torch(
                layer_head_mask_reshaped.to(dtype=torch.bfloat16),
                dtype=ttnn.bfloat16,
                layout=ttnn.TILE_LAYOUT,
                device=device,
            )
        else:
            layer_head_mask_ttnn = layer_head_mask

        # Multiply attention weights by head mask (0 masks out the head, 1 keeps it)
        attn_weights = ttnn.mul(attn_weights, layer_head_mask_ttnn)

    # Apply attention to values: attention_weights * V with high precision
    ...
    # Apply output projection with HiFi4 precision
