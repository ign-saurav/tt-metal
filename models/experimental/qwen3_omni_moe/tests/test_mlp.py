...
"""
PCC test comparing torch reference and TTNN output for Qwen3-Omni-MoE vision MLP.
No HF_MODEL required; uses standalone vision-only config.
"""
import os
import sys
...
from tt.tt_vision_mlp import MLP
...
def test_mlp_inference(rows, batch_size, mesh_device, reset_seeds, ensure_gc):
    ...
    model_args = Qwen3OmniVisionModelArgs(mesh_device, dummy_weights=True, max_batch_size=batch_size, max_seq_len=rows)
