...
class DetNuscEvaluator:
    ...
    def format_results(self, results, img_metas, result_names=["img_bbox"], jsonfile_prefix=None, **kwargs):
        ...
        for rasult_name in result_names:
            ...
            if self.output_dir:
                result_files.update({rasult_name: self._format_bbox(results, img_metas, self.output_dir)})
            else:
                result_files.update({rasult_name: self._format_bbox(results, img_metas, tmp_file_)})
    ...
    def evaluate(
        ...
        """
        result_files, tmp_dir = self.format_results(results, img_metas, result_names, jsonfile_prefix)
