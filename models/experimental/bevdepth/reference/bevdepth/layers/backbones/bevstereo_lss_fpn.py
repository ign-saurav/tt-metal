...
from bevdepth.layers.backbones.base_lss_fpn import ASPP, BaseLSSFPN, Mlp, SELayer
...
class BEVStereoLSSFPN(BaseLSSFPN):
    def __init__(
        ...
        super(BEVStereoLSSFPN, self).__init__(
            x_bound,
            y_bound,
            z_bound,
            d_bound,
        ...
        if range_list is None:
            ...
            self.range_list = [
                [d_bound[0] + range_length * i, d_bound[0] + range_length * (i + 1)] for i in range(num_ranges)
            ]
        else:
    ...
    def _generate_cost_volume(
        ...
        for idx in range(num_sweeps):
            ...
            feat_cost = torch.mean((ref_stereo_feat.unsqueeze(3) * warped_stereo_fea), axis=2)
    ...
    def homo_warping(
        ...
        with torch.no_grad():
            ...
            points = (
                sweep_intrin_mats.reshape(batch_size_with_num_cams, *sweep_intrin_mats.shape[2:]).unsqueeze(1) @ points
            )
        ...
        warped_stereo_fea = warped_stereo_fea.view(batch_size_with_num_cams, channels, num_depth, height, width)
    ...
    def _forward_stereo(
        ...
        mask_score = stereo_feats_all_sweeps[0].new_zeros(
            ...
            self.depth_channels,
            feat_height * self.stereo_downsample_factor // self.downsample_factor,
            feat_width * self.stereo_downsample_factor // self.downsample_factor,
        )
    ...
    def _forward_single_sweep(self, sweep_index, context, mats_dict, depth_score, is_return_depth=False):
        ...
        if self.training or self.use_da:
            ...
            feature_map = voxel_pooling_inference(
                geom_xyz, depth.contiguous(), context.contiguous(), self.voxel_num.cuda()
            )
        if is_return_depth:
    ...
    def forward(self, sweep_imgs, mats_dict, timestamps=None, is_return_depth=False):
        ...
        for sweep_index in range(0, num_sweeps):
            if sweep_index > 0:
                with torch.no_grad():
                    ...
                    depth_feat, context, mu, sigma, range_score, mono_depth = self.depth_net(
        ...
        for ref_idx in range(num_sweeps):
            ...
            if self.use_mask:
                depth_score = (mono_depth_all_sweeps[ref_idx] + self.depth_downsample_net(stereo_depth) * mask).softmax(
                    1, dtype=stereo_depth.dtype
                )
            else:
                depth_score = (mono_depth_all_sweeps[ref_idx] + self.depth_downsample_net(stereo_depth)).softmax(
                    1, dtype=stereo_depth.dtype
                )
            depth_score_all_sweeps.append(depth_score)
            if ref_idx == 0:
                ...
                final_depth = (mono_depth_all_sweeps[ref_idx] + self.depth_downsample_net(stereo_depth)).softmax(1)
