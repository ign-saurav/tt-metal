...
from models.experimental.bevdepth.reference.bevdepth.layers.backbones.base_lss_fpn import BasicBlock
...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mats_dict, lidar_depth, scale_depth_factor=1000.0):
        ...
        aug_scale = torch.sqrt(
            mats_dict["ida_mats"][:, 0, :, 0, 0] ** 2 + mats_dict["ida_mats"][:, 0, :, 0, 0] ** 2
        ).reshape(-1, 1)
...
class FusionLSSFPN(BaseLSSFPN):
    ...
    def _forward_single_sweep(self, sweep_index, sweep_imgs, mats_dict, sweep_lidar_depth, is_return_depth=False):
    ...
    def forward(self, sweep_imgs, mats_dict, lidar_depth, timestamps=None, is_return_depth=False):
        """Forward function.
        ...
        key_frame_res = self._forward_single_sweep(
            0, sweep_imgs[:, 0:1, ...], mats_dict, lidar_depth[:, 0, ...], is_return_depth=is_return_depth
        )
        if num_sweeps == 1:
    ...
    def get_downsampled_lidar_depth(self, lidar_depth):
        ...
        lidar_depth = lidar_depth.view(-1, self.downsample_factor * self.downsample_factor)
        gt_depths_tmp = torch.where(lidar_depth == 0.0, lidar_depth.max(), lidar_depth)
        lidar_depth = torch.min(gt_depths_tmp, dim=-1).values
        lidar_depth = lidar_depth.view(
            batch_size, num_sweeps, num_cams, 1, height // self.downsample_factor, width // self.downsample_factor
        )
