...
class MatrixVT(BaseLSSFPN):
    ...
    def get_proj_mat(self, mats_dict=None):
        ...
        invalid2 = torch.logical_or((geom_sep > (bev_size - 1))[..., 0], (geom_sep > (bev_size - 1))[..., 1])
    ...
    def reduce_and_project(self, feature, depth, mats_dict):
        ...
        img_feat_with_depth = img_feat_with_depth.permute(0, 2, 1).reshape(B, -1, *self.voxel_num[:2])
    ...
    def _forward_single_sweep(self, sweep_index, sweep_imgs, mats_dict, is_return_depth=False):
        ...
        with autocast(enabled=False):
            feature = depth_feature[:, self.depth_channels : (self.depth_channels + self.output_channels)].float()
            depth = depth_feature[:, : self.depth_channels].float().softmax(1)

            img_feat_with_depth = self.reduce_and_project(feature, depth, mats_dict)  # [b*n, c, d, w]
