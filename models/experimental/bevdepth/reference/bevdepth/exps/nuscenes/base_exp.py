...
from models.experimental.bevdepth.reference.bevdepth.evaluators.det_evaluators import DetNuscEvaluator
from models.experimental.bevdepth.reference.bevdepth.models.base_bev_depth import BaseBEVDepth
from models.experimental.bevdepth.reference.bevdepth.utils.torch_dist import all_gather_object, get_rank, synchronize
...
class BEVDepthLightningModel(LightningModule):
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        all_pred_results = sum(map(list, zip(*all_gather_object(all_pred_results))), [])[:dataset_length]
        all_img_metas = sum(map(list, zip(*all_gather_object(all_img_metas))), [])[:dataset_length]
        if get_rank() == 0:
            self.evaluator.evaluate(all_pred_results, all_img_metas)
