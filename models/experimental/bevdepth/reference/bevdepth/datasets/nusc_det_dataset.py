...
import numpy as np
import torch
from models.experimental.bevdepth.reference.bevdepth.datasets.lidarinstance3dboxes import LiDARInstance3DBoxes
...
def map_pointcloud_to_image(
    ...
    # + renormalization).
    points = view_points(
        lidar_points.points[:3, :], np.array(cam_calibrated_sensor["camera_intrinsic"]), normalize=True
    )
...
class NuscDetDataset(Dataset):
    def __init__(
        ...
        assert sum([key_idx < 0 for key_idx in key_idxes]) == len(key_idxes), "All `key_idxes` must less than 0."
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                if self.return_depth and (self.use_fusion or sweep_idx == 0):
                    point_depth = self.get_lidar_depth(
                        sweep_lidar_points[sweep_idx], img, lidar_infos[sweep_idx], cam_info[cam]
                    )
    ...
    def __getitem__(self, idx):
        ...
        for key_idx in self.key_idxes:
            ...
            lidar_sweep_timestamps = [lidar_sweep["LIDAR_TOP"]["timestamp"] for lidar_sweep in info["lidar_sweeps"]]
