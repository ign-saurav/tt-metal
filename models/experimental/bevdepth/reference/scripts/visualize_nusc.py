...
def demo(
    ...
    lidar_points = np.fromfile(os.path.join("data/nuScenes", lidar_path), dtype=np.float32, count=-1).reshape(-1, 5)[
        ..., :4
    ]
    ...
    for i, k in enumerate(IMG_KEYS):
        ...
        img = mmcv.imread(os.path.join("data/nuScenes", info["cam_infos"][k]["filename"]))
