...
class TestLSSFPN(unittest.TestCase):
    @pytest.mark.skipif(condition=torch.cuda.is_available() is False, reason="No gpu available.")
    def test_voxel_pooling(self):
        ...
        bev_featuremap = voxel_pooling_train(
            geom_xyz.cuda().int(), features.cuda(), torch.tensor([128, 128, 1], dtype=torch.int, device="cuda")
        )
        assert torch.allclose(gt_bev_featuremap.cuda(), bev_featuremap, rtol=1e-3)
