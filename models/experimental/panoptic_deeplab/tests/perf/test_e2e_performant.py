...
def run_model_pipeline(device, test_infra, num_measurement_iterations):
    ...
    def model_wrapper(input_tensor):
        ...
        test_infra.run()
        return test_infra.tt_output_sem_seg, test_infra.tt_output_ins_seg_offset, test_infra.tt_output_ins_seg_center
...
def run_perf_e2e_panoptic_deeplab(
    device,
    ...
    test_infra = PanopticDeepLabPerformanceRunnerInfra(
        device,
        batch_size,
        act_dtype,
        weight_dtype,
    ...
    prep_perf_report(
        model_name=f"ttnn_panoptic_deeplab_trace_2cqs_batch_size{batch_size}",
    ...
    logger.info(
        f"Panoptic Deeplab {resolution[0]}x{resolution[1]} batch_size: {batch_size}, inference time (avg): {inference_time_avg}, FPS: {batch_size/inference_time_avg}"
    )
    logger.info(f"Panoptic Deeplab compile time: {compile_time} s")
...
@pytest.mark.parametrize(
    "batch_size_per_device, act_dtype, weight_dtype, math_fidelity",
