...
class Demo:
    ...
    def run_demo(self, image_path: str, output_dir: str) -> None:
        ...
        ttnn_output = self.run_ttnn_inference(self.ttnn_input, model_config, device)
        # Compare the accuracy of torch_output vs ttnn_output
        # For simplicity, compare regression and classification outputs using PCC
        # (as done in test_retinanet). Log the results.

        def check_with_pcc(ref: Tensor, out: Tensor, pcc=0.99):
            # Compute PCC between two torch tensors, flattening them
            x = ref.flatten().float()
            y = out.flatten().float()
            vx = x - torch.mean(x)
            vy = y - torch.mean(y)
            denom = torch.sqrt(torch.sum(vx**2)) * torch.sqrt(torch.sum(vy**2)) + 1e-12
            pcc_score = torch.sum(vx * vy) / denom
            pcc_passed = pcc_score.item() >= pcc
            pcc_message = f"PCC={pcc_score.item():.5f} (pass: {pcc_passed}, thr: {pcc})"
        ...
        for key in ["regression", "classification"]:
            ...
            ttnn_tensor_torch = ttnn_output[key]
            # Convert ttnn host/device output to torch tensor if needed
            # if hasattr(ttnn_tensor, "to_torch"):  # TTNN tensor
            #     ttnn_tensor_torch = ttnn_tensor.to_torch()  # (for newer ttnn)
            # else:
            #     # This assumes ttnn.to_torch available in your env
            #     ttnn_tensor_torch = ttnn.to_torch(ttnn_tensor)
            # Compare (reshape to match, if necessary)
            if torch_tensor.shape != ttnn_tensor_torch.shape:
                # Sometimes outputs are NCHW vs NHWC etc. Try to permute/reshape if needed.
                # But here we assume direct match or rely on test infra shape as test_retinanet.py does.
                ttnn_tensor_flat = ttnn_tensor_torch.flatten()
                torch_tensor_flat = torch_tensor.flatten()
            else:
                ttnn_tensor_flat = ttnn_tensor_torch.flatten()
                torch_tensor_flat = torch_tensor.flatten()
            passed, msg = check_with_pcc(torch_tensor_flat, ttnn_tensor_flat, pcc=pcc_thresholds[key])
            pcc_results[key] = msg
            logger.info(f"[PCC] {key}: {msg}")
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input", "-i", default="models/experimental/retinanet/resources/dog_800x800.jpg", help="Path to input image"
    )
    parser.add_argument(
        "--output",
        "-o",
