...
from models.experimental.retinanet.TTNN.tt_retinanet import TTRetinanet
from models.experimental.retinanet.TTNN.custom_preprocessor import create_custom_mesh_preprocessor


class RetinaNetCore(torch.nn.Module):
    def __init__(self, backbone, classification_head, regression_head):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # Each FPN level (P3–P7)
        for f in features.values():
            cls_out = self.classification_head(f)
            reg_out = self.regression_head(f)

            # Flatten + permute each (same as official RetinaNet)
            cls_out = cls_out.flatten(start_dim=2).permute(0, 2, 1)
            reg_out = reg_out.flatten(start_dim=2).permute(0, 2, 1)

            cls_outputs.append(cls_out)
            reg_outputs.append(reg_out)

        # Concat all levels
        cls_outputs = torch.cat(cls_outputs, dim=1)  # [N, total_anchors, num_classes * 9]
        reg_outputs = torch.cat(reg_outputs, dim=1)  # [N, total_anchors, 4 * 9]
...
class RetinanetTestInfra:
    def __init__(self, device, batch_size, in_channels, height, width, model_config, name):
        ...
        retinanet.eval()
        resnet_backbone = retinanet.backbone
        classification = retinanet.head.classification_head
        regression = retinanet.head.regression_head
        torch_model = RetinaNetCore(resnet_backbone, classification, regression)
        ...
        # TTNN model
        self.ttnn_model = TTRetinanet(parameters=parameters, model_config=model_config, device=device)
    ...
    def validate(self, output_tensor=None):
        ...
        valid_pcc = {"classification": 0.9, "regression": 0.9}
        self.pcc_passed_all = []
