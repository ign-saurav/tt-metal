...
# Create a config for GemmaForCausalLM
...
# Create the model (this uses whatever version is currently installed/patched)
model = PaliGemmaForConditionalGeneration(config=config)

print("="*80)
print("CURRENT PaliGemmaForConditionalGeneration Model Structure (from HuggingFace transformers)")
print("="*80)
print(model)
